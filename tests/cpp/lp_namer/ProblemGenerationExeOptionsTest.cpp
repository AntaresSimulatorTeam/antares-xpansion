...
class ProblemGenerationExeOptionsTest
    : public testing::TestWithParam<
        std::tuple<std::pair<std::string, std::string>, std::pair<std::string, std::string>>>
{
public:
    ...
    auto parseOptions(std::convertible_to<std::string_view> auto&&... s)
    {
        ...
        return problem_generation_options_parser_.Parse(ppargv.size(), ppargv.data());
    }
};
...
public:
    ...
    void RunProblemGeneration(const std::filesystem::path& xpansion_output_dir,
...
private:
    ...
    void set_solver(std::filesystem::path study_dir,
...
TEST_F(ProblemGenerationExeOptionsTest, MasterFormulationDefaultValue)
{
    ...
    ASSERT_EQ(problem_generation_options_parser_.MasterFormulation(), std::string("relaxed"));
}
...
{
    return std::tuple_cat(std::tuple(pair.first, pair.second), flattenPairs(rest...));
}

auto cases()
{
    using namespace std::string_literals;
    return ::testing::Combine(::testing::Values(std::pair("--archive"s, "archive.zip"s),
...
TEST_P(ProblemGenerationExeOptionsTest, Parameters_mutually_exclusives)
{
    ...
    auto tuple = std::apply([](auto... args) { return flattenPairs(args...); }, params);
    EXPECT_THROW(std::apply([&, this](auto... args) { parseOptions(args...); }, tuple),
                 ProblemGenerationOptions::ConflictingParameters);

    EXPECT_THROW(parseOptions("--archive",
                              archive,
                              "--output",
                              output_path.string(),
                              "--study",
...
INSTANTIATE_TEST_SUITE_P(ConflictingOptions, ProblemGenerationExeOptionsTest, cases());

TEST_F(ProblemGenerationExeOptionsTest, OutputAndArchiveParameters_deduceOuputFromArchive)
{
    auto test_root = std::filesystem::temp_directory_path() / std::tmpnam(nullptr);
    ...
    EXPECT_EQ(pbg.configurationManager().Directories().archive_path, archive);
    EXPECT_EQ(pbg.configurationManager().Directories().xpansion_output_dir, output_path);
    EXPECT_TRUE(std::filesystem::exists(output_path));
    EXPECT_TRUE(std::filesystem::exists(output_path / "lp"));
}

TEST_F(ProblemGenerationExeOptionsTest, ValuesAndXpansionDirExistsWhenUsingOutputOption)
{
    auto test_root = std::filesystem::temp_directory_path() / std::tmpnam(nullptr);
    ...
    EXPECT_EQ(pbg.configurationManager().Directories().xpansion_output_dir,
              simulation_path.string());
    EXPECT_TRUE(
      std::filesystem::exists(pbg.configurationManager().Directories().xpansion_output_dir / "lp"));
}
