...
protected:
    void SetUp() override
    {
        ...
        problems_and_data = {
          {1, ProblemData("problem-1-1--optim-nb-1.mps", "variables.txt")},
          {1, ProblemData("problem-1-50--optim-nb-1.mps", "variables.txt")},
          {2, ProblemData("problem-2-10--optim-nb-1.mps", "variables.txt")},
          {2, ProblemData("problem-2-11--optim-nb-1.mps", "variables.txt")},
          {2, ProblemData("problem-2-30--optim-nb-1.mps", "variables.txt")},
          {3, ProblemData("problem-3-20--optim-nb-1.mps", "variables.txt")},
        };
    }

    void RunWeightsFileWriterTest(const std::string& solver_name, const std::string& expected)
    {
        auto yearly_weight_writer = YearlyWeightsWriter(tempDir,
                                                        {3, 5, 7},
                                                        "weights_123.txt",
                                                        {1, 2, 3},
                                                        solver_name,
