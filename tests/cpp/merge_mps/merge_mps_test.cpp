...
ENDATA)";
    ...
    /**
     * Create the following satellite problem :
     * Minimize
     * Obj: 1Y1
     * s.t.
     * 3Y1 + 2X1 <= 7
...
// Following tests represent the merged problem
/**
 * Expectation :
\Problem name: ClpDefaultName
...
TEST_F(MergeMPSTest, merged_problems_with_zero_weight)
{
    ...
    // Satellite contribution to objective should be zero
    auto master = factory.create_solver("CBC");
    master->read_prob_mps(tmp_dir_ / "master.mps"s);
...
ENDATA)";
    ...
    // Should have proper number of variables
...
TEST_F(MergeMPSTest, output_lp_format_not_generated_by_default)
{
    ...
    // LP file should not be generated by default
    EXPECT_FALSE(std::filesystem::exists(tmp_dir_ / "log_merged.lp"s));
    // But MPS file should exist
    EXPECT_TRUE(std::filesystem::exists(tmp_dir_ / "log_merged.mps"s));
}
...
ENDATA)";
    ...
    // Each satellite should have weight 1/2 (2 satellites)
    SolverFactory factory;
    auto merged = factory.create_solver("CBC");
    merged->read_prob_mps(tmp_dir_ / "log_merged.mps"s);
...
TEST_F(MergeMPSTest, constant_slave_weight_mode)
{
    ...
    // Create satellite manually without setting weight
    std::ofstream satellite_problem(tmp_dir_ / "satellite.mps"s);
    satellite_problem << R"(NAME       SATELLITE  FREE
ROWS
 N  OBJROW
...
ENDATA)";
    ...
    // Test CONSTANT weight mode
    options_.SLAVE_WEIGHT = "CONSTANT";
    options_.SLAVE_WEIGHT_VALUE = 52; // Each weight will be 1/52
...
TEST_F(MergeMPSTest, missing_weight_warning_zero_contribution)
{
    ...
    // Don't set weight for satellite - should default to 0 with warning
    ...
    // Satellite contribution should be zero
    for (int i = master->get_ncols(); i < merged->get_ncols(); ++i)
    {
        EXPECT_DOUBLE_EQ(obj_merged[i], 0.0);
    }
...
TEST_F(MergeMPSTest, weight_near_one_skips_multiplication)
{
    ...
    // Weight very close to 1 (within tolerance) should skip multiplication
    ...
    // Should be unchanged (weight optimization kicks in)
    for (int i = 0; i < satellite->get_ncols(); ++i)
    {
...
TEST_F(MergeMPSTest, solver_name_coin_mapped_to_cbc)
{
    ...
    // Use "COIN" which should be mapped to "CBC"
    options_.SOLVER_NAME = "COIN";
...
TEST_F(MergeMPSTest, coupling_constraints_with_variable_in_three_problems)
{
    ...
    // Create third problem sharing X1
    std::ofstream third_problem(tmp_dir_ / "third.mps"s);
    third_problem << R"(NAME       THIRD  FREE
ROWS
 N  OBJROW
...
ENDATA)";
    ...
    // X1 appears in all three problems
    createStructureFile({{"master.mps", "X1", 0},
                         {"master.mps", "X2", 1},
                         {"satellite.mps", "X1", 1},
                         {"third.mps", "X1", 1}});
    ...
    // X1 appears in 3 problems, so we need 2 coupling constraints for X1
    EXPECT_EQ(merged->get_nrows(),
