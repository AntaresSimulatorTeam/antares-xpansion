...
public:
    ...
    void verify_candidates_writing(const Point &candidates, const Json::Value &written_candidates)
    {
        ...
        for (auto &kvp : candidates)
        {
            ...
            ASSERT_DOUBLE_EQ(kvp.second, written_candidates[candidate_id][INVEST_C].asDouble());
            candidate_id++;
        }
    }

    void verify_pbs_data_writing(const std::vector<SinglePbData> &pbs_data, const Json::Value &written_pbs_data)
    {
        ...
        for (int pb_id(0); pb_id < pbs_data.size(); pb_id++)
        {
            ...
            ASSERT_DOUBLE_EQ(pbs_data[pb_id].system_cost, written_pbs_data[pb_id][SYSTEM_COST_C].asDouble());

            verify_candidates_writing(pbs_data[pb_id].candidates, written_pbs_data[pb_id][CANDIDATES_C]);
        }
    }

    void verify_output_writing(const SensitivityOutputData &output_data, Json::Value written_data)
    {
        ASSERT_EQ(output_data.epsilon, written_data[EPSILON_C].asDouble());
        ASSERT_EQ(output_data.best_benders_cost, written_data[BEST_BENDERS_C].asDouble());

        verify_pbs_data_writing(output_data.pbs_data, written_data[SENSITIVITY_SOLUTION_C]);
    }
};
...
TEST_F(SensitivityWriterTest, EndWritingPrintsOutputData)
{
    ...
    SinglePbData capex_min_data = {CAPEX_C,
                                   MIN_C,
                                   10,
                                   100,
                                   {{peak_name, 20}, {semibase_name, 50}},
                                   0};
    SinglePbData proj_max_data = {PROJECTION_C,
                                  MAX_C,
                                  200,
                                  500,
