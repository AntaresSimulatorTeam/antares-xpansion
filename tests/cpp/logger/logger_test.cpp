...
void addCandidate(LogData& logData,
...
const LogData best_iteration_data = {15e5,
                                     255e6,
                                     200e6,
                                     63,
                                     63,
...
public:
    ...
    void TearDown()
    {
        std::remove(_fileName.string().c_str());
    }
...
TEST_F(FileLoggerTest, ShouldPrintRestartMessage)
{
    ...
    stringStreamFromFile << fileStream.rdbuf();
    fileStream.close();
    auto stringFromFileWithoutPrefix = RemovePrefixFromLineMessage(stringStreamFromFile.str());
...
TEST_F(FileLoggerTest, ShouldPrintRestartElapsedTimeMessage)
{
    ...
    const double elapsed_time_numeric(days * 24 * 60 * 60 + hours * 60 * 60 + minutes * 60
                                      + seconds);
    const std::string elapsed_time_str(std::to_string(days) + " days " + std::to_string(hours)
...
TEST_F(FileLoggerTest, ShouldPrintNumberOfIterationsBeforeRestart)
{
    ...
    expected_msg << " \tNumber of Iterations performed: " << num_iteration << std::endl;
    UserFile fileLog(_fileName);
...
TEST_F(FileLoggerTest, ShouldPrintBestIterationsInfos)
{
    ...
    expected_msg << " \tBest Iteration Infos: " << std::endl
                 ...
                 << std::endl
                 << " " << indent_1 << "Subproblems solved in (walltime): " << l.subproblem_time
                 << " s" << std::endl
                 << " " << indent_0 << "Solution =" << std::endl
                 << " " << indent_0 << indent_1 << "Operational cost =       15.50 Me" << std::endl
                 << " " << indent_0 << indent_1 << " Investment cost =       20.00 Me" << std::endl
                 << " " << indent_0 << indent_1 << "    Overall cost =       35.50 Me" << std::endl
                 << " " << indent_0 << indent_1 << "   Best Solution =        3.00 Me" << std::endl
                 << " " << indent_0 << indent_1 << "     Lower Bound =        2.00 Me" << std::endl
                 << " " << indent_0 << indent_1 << "    Absolute gap = 1.00000e+06 e" << std::endl
                 << " " << indent_0 << indent_1 << "    Relative gap = 3.33333e-01" << std::endl;
    UserFile fileLog(_fileName);
    ...
    auto stringFromFileWithoutPrefix = RemovePrefixFromMessage(stringStreamFromFile);
...
TEST_F(UserLoggerTest, InvalidStreamNotified)
{
    ...
    auto logWithoutPrefix = RemovePrefixFromLineMessage(redirectedErrorStream.str());
...
TEST_F(UserLoggerTest, IterationStartLogCandidateOrder)
{
    ...
    expected << " " << indent_0 << indent_1
             << "a = 10.00 invested MW -- possible interval [0.00;  50.00] MW" << std::endl;
    expected << " " << indent_0 << indent_1
             << "c = 50.00 invested MW -- possible interval [0.00; 100.00] MW" << std::endl;
    expected << " " << indent_0 << indent_1
             << "z = 50.00 invested MW -- possible interval [0.00; 100.00] MW" << std::endl;
...
TEST_F(UserLoggerTest, EndLog)
{
    ...
    std::stringstream expected;
    expected << " " << indent_1 << "Total number of iterations done = " << logData.it << std::endl;
...
TEST_F(UserLoggerTest, CumulativeNumberOfSubProblemResolved)
{
    ...
    auto logWithoutPrefix = RemovePrefixFromMessage(_stream);
    std::stringstream expected;
    expected << " " << indent_1 << "cumulative number of subproblem resolutions: " << number
             << std::endl;
    ASSERT_EQ(logWithoutPrefix, expected.str());
}
...
{
    std::stringstream expected;
    expected << " --- Relaxed gap reached, switch master formulation to integer" << std::endl;
...
TEST(LogDestinationTest, WithInvalidEmptyFilePath)
{
    ...
    expected_msg << " Could not open the file: " << std::quoted(invalid_file_path.string().c_str())
                 << "\n";
...
TEST(LogDestinationTest, StdoutWithAValidMessage)
{
    ...
    const std::string expected_msg = msg + std::string((size_t)indentation - msg.size(), ' ');
...
TEST(LogDestinationTest, MessageWithAValidFile)
{
    ...
    auto log_file = CreateRandomSubDir(std::filesystem::temp_directory_path()) / "log.txt";
