...
#include "../CoreHexagone/Model/Xpansion/Area.h"
...
class XpansionStudyFileAdapter : public IForProvidingXpansionStudyPort {
 public:
  explicit XpansionStudyFileAdapter(
      const StudyFileReader::ILinkFileReader &link_file_reader);
  explicit XpansionStudyFileAdapter(
      const StudyFileReader::IAreaFileReader &area_file_reader);
  explicit XpansionStudyFileAdapter(
      const StudyFileReader::ICandidateFileReader &candidate_reader);

  XpansionStudyFileAdapter(
      std::shared_ptr<StudyFileReader::ILinkFileReader> link_reader,
