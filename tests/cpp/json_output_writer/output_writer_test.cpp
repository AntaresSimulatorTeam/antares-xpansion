...
TEST_F(JsonWriterTest, InitialiseShouldPrintBeginTimeAndOptions) {
  ...
  ASSERT_EQ(benders_options.LOG_LEVEL,
            json_content[OPTIONS_C]["LOG_LEVEL"].asInt());
  ASSERT_EQ(benders_options.MAX_ITERATIONS,
            json_content[OPTIONS_C]["MAX_ITERATIONS"].asInt());
  ASSERT_EQ(benders_options.ABSOLUTE_GAP,
            json_content[OPTIONS_C]["ABSOLUTE_GAP"].asDouble());
  ASSERT_EQ(benders_options.RELATIVE_GAP,
            json_content[OPTIONS_C]["RELATIVE_GAP"].asDouble());
  ASSERT_EQ(benders_options.AGGREGATION,
            json_content[OPTIONS_C]["AGGREGATION"].asBool());
  ASSERT_EQ(benders_options.OUTPUTROOT,
            json_content[OPTIONS_C]["OUTPUTROOT"].asString());
  ASSERT_EQ(benders_options.TRACE, json_content[OPTIONS_C]["TRACE"].asBool());
  ASSERT_EQ(benders_options.SLAVE_WEIGHT,
            json_content[OPTIONS_C]["SLAVE_WEIGHT"].asString());
  ASSERT_EQ(benders_options.MASTER_NAME,
            json_content[OPTIONS_C]["MASTER_NAME"].asString());
  ASSERT_EQ(benders_options.SLAVE_NUMBER,
            json_content[OPTIONS_C]["SLAVE_NUMBER"].asInt());
  ASSERT_EQ(benders_options.INPUTROOT,
            json_content[OPTIONS_C]["INPUTROOT"].asString());
  ASSERT_EQ(benders_options.CSV_NAME,
            json_content[OPTIONS_C]["CSV_NAME"].asString());
  ASSERT_EQ(benders_options.BOUND_ALPHA,
            json_content[OPTIONS_C]["BOUND_ALPHA"].asBool());
  ASSERT_EQ(benders_options.SOLVER_NAME,
            json_content[OPTIONS_C]["SOLVER_NAME"].asString());
  ASSERT_EQ(benders_options.JSON_FILE,
            json_content[OPTIONS_C]["JSON_FILE"].asString());
  ASSERT_EQ(benders_options.TIME_LIMIT,
            json_content[OPTIONS_C]["TIME_LIMIT"].asDouble());
}
