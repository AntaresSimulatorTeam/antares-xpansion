...
{
	AdditionalConstraintsReader additionalConstraintsReader_l("temp_additional_constraints.ini");

	std::set<std::string> sections_l = additionalConstraintsReader_l.getSections();

	std::set<std::string> expectedSections_l = {"binary_constraint_3", "variables", "1", "2", "10"};
	ASSERT_EQ(expectedSections_l.size(), sections_l.size());

	auto expected_it = expectedSections_l.begin();
	for (auto sections_it = sections_l.begin(); sections_it != sections_l.end(); ++sections_it)
	{
...
TEST_F(AdditionalConstraintsReaderTest, testVariables)
{
	...
	std::map<std::string, std::string> variablesSection_l =  additionalConstraintsReader_l.getVariablesSection();
	...
	for (auto vars_it = variablesSection_l.begin(); vars_it != variablesSection_l.end(); ++vars_it)
	{
...
TEST_F(AdditionalConstraintsReaderTest, testOneSection)
{
	...
	std::map<std::string, std::string> section_l =  additionalConstraintsReader_l.getSection("10");
	...
	for (auto vals_it = section_l.begin(); vals_it != section_l.end(); ++vals_it)
	{
