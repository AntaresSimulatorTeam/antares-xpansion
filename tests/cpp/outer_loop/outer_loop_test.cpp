...
const auto STUDY_PATH = std::filesystem::path("data_test") / "external_loop_test";
...
INSTANTIATE_TEST_SUITE_P(availsolvers, MasterUpdateBaseTest, ::testing::ValuesIn(solvers()));
...
TEST_F(OuterLoopPatternTest, RegexGivenPrefixAndBody)
{
    ...
    ASSERT_EQ((prefix + "::" + body + "::suffix").find(ret_regex) != std::string::npos, false);
    ...
    ASSERT_EQ((prefix + "area<" + body + ">::suffix").find(ret_regex) != std::string::npos,
              true); // Match
    ASSERT_EQ((prefix + "area<" + body + "_other_area>::suffix").find(ret_regex)
                != std::string::npos,
              false);
}
...
{
    std::filesystem::path empty(std::filesystem::temp_directory_path() / "empty.yml");
...
{
    std::filesystem::path patterns_not_array(std::filesystem::temp_directory_path()
                                             / "patterns_not_array.yml");
...
{
    std::filesystem::path valid_file(std::filesystem::temp_directory_path() / "valid_file.yml");
...
{
    std::vector<std::string> variables{"Gold::area<Sun>::hour<9999>",
                                       "Blue::area<Earth>::hour<125>",
                                       "Red::area<Mars>::hour<1546>",
                                       "Blue::area<Earth>::hour<3336>"};
...
{
    Outerloop::OuterLoopBiLevel outerLoopBiLevel(data);
    const std::vector<double> criteria = {data[0].Criterion() + 12, data[1].Criterion() + 2024};
    const auto lambda = 1205;
    ASSERT_FALSE(outerLoopBiLevel.Update_bilevel_data_if_feasible({}, criteria, 0., 0., lambda));
...
{
    Outerloop::OuterLoopBiLevel outerLoopBiLevel(data);
    const std::vector<double> criteria = {data[0].Criterion() - 12, data[1].Criterion() - 24};
    const auto lambda = 1205;
    const auto overall_cost = 1e20;
    ASSERT_FALSE(
      outerLoopBiLevel.Update_bilevel_data_if_feasible({}, criteria, overall_cost, 0., lambda));
    ASSERT_EQ(outerLoopBiLevel.LambdaMin(), lambda);
}
...
TEST_F(OuterLoopBiLevelTest, FeasibleScenario)
{
    ...
    ASSERT_TRUE(outerLoopBiLevel.Update_bilevel_data_if_feasible(x,
                                                                 criteria,
                                                                 overall_cost,
                                                                 invest_cost_at_x,
                                                                 lambda));
...
TEST_F(OuterLoopBiLevelTest, FeasibleScenario_InvestCostAtX_Higher_Than_Initial_LambdaMax)
{
    const auto lambda = 1205;
    const auto overall_cost = 105;
    const auto cand = "candidate";
