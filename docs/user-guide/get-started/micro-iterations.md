...
typically one week — at hourly resolution. Representing the system at a 1-hour
time step means that every operational constraint is instantiated for each of
the 168 hours of the week: supply–demand balance in every area, flow bounds on
every interconnection, technical limits of thermal clusters, hydraulic storage
management, as well as user-defined binding constraints. The size of the
resulting linear programs grows accordingly, and for large interconnected
studies each subproblem becomes a heavy optimization problem in its own right,
with a resolution time that is far from negligible. Since a classical Benders
iteration requires solving *all* subproblems before the master problem can be
enriched with new cuts, the subproblem phase concentrates the vast majority of
the computational effort, and the cost of a single full iteration becomes the
limiting factor of the whole resolution. The *micro-iterations* paradigm was
introduced to attack precisely this bottleneck.
...
faster to solve. This relaxation is solved a first time, and the resulting
solution is handed to a verification mechanism whose role is to check, outside
of the optimization itself, whether the constraints that were left out of the
model are satisfied by the current solution. The constraints found to be
violated are injected into the subproblem, which is then re-solved with this
enriched formulation. This solve–check–inject sequence constitutes one
*micro-iteration*, and it is repeated until the verification step detects no
further violation. At that point, the solution of the relaxed problem is
feasible for the complete formulation — and therefore optimal for it, since
removing constraints can only relax the problem. The subproblem thus returns
exactly the same optimal value and dual information as if it had been solved
with all constraints from the start, but the solver only ever handles the
constraints that actually matter: in practice, a large share of the operational
constraints are inactive at the optimum, and the successive relaxations remain
far smaller than the full problem.
