...
does. Yet this cost is largely redundant: the subproblems all derive from the
same Antares study and therefore share a huge part of their columns and rows —
the same variables, the same constraint structure — differing only in the data
that is specific to each time frame. The idea of the memory optimization is to
exploit this redundancy by no longer providing one MPS file per subproblem, but
a single MPS file acting as a *skeleton*: the common structure shared by all
subproblems. This skeleton is complemented by a set of much lighter files, one
per subproblem, containing only the subproblem-specific data needed to
instantiate the actual problem at resolution time. Memory then holds one copy
of the shared structure instead of one full problem per subproblem, and the
per-subproblem cost is reduced to its lightweight specialization data.
