...
| -----| -------------| -------------|-------------|
|MAX_ITERATIONS | `-1` | The maximum number of Benders iteration |`integer` |
|RELATIVE_GAP | `1e-6` | Tolerance on relative gap  | `double` |
|ABSOLUTE_GAP | `1` | Tolerance on absolute gap  |`double` |
|RELAXED_GAP | `1e-5` | Level of precision with master relaxation   |`double` |
...
|SEPARATION_PARAM | `0.5` | In-out separation parameter |`double` |
...
|MASTER_FORMULATION | `integer` | Formulation of the master problem | `integer`|
|SOLVER_NAME | `COIN` | Name of solver to use | `XPRESS`, `COIN`|
|TIME_LIMIT | `1e12` | Simulation time limit | `double` |
|LOG_LEVEL | `0` | The degree of detail of the output, from 0 to 2 |from `0` to `2` |
...
|MASTER_SOLUTION_TOLERANCE | `1e-4` |  Tolerance for rounding the solution variables of the master problem (to avoid subproblems infeasibilities) | `double`|
|CUT_COEFFICIENT_TOLERANCE | `5e-3` |  Cofficient under which cuts coefficients and right-hand sides are considered to be zero | `double` |
