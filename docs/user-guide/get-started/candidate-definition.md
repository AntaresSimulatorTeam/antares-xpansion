...
With Antares simulator 8.2 users can define different NTC chronicles for different Monte-Carlo year. Briefly, users defines any number
...
1) On a candidate, direct and indirect link profiles must have the same number of column
2) The same rule applies for already installed direct and indirect profile
3) The number of column in link profile and already installed profiles must be the same
4) The number of column for NTC chronicles on a given link must be the same as the number of column
