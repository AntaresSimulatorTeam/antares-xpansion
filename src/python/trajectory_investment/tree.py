...
from trajectory_input_keys import ConstraintType, ConstraintOperator, InvestmentVariableType
...
class TrajectoryTreeModule:
    ...
    class TrajectoryGlobalData:
        ...
        def __init__(self, yaml_data_global, yaml_data_capacities):
            self.discount_rate = yaml_data_global.get(TrajectoryInputKeys.discount_rate_key(), 0.0)
            self.first_investment_year = yaml_data_global.get(TrajectoryInputKeys.first_investment_year_key(), 2025)
            # Path to studies
    ...
    class TreeNode:
        ...
        def __init__(self, yaml_data):
            ...
            self.probability_from_parent = yaml_data.get(TrajectoryInputKeys.probability_key(), 1.0)
            ...
            if len(children_yaml) > 0:
                self.children = [TrajectoryTreeModule.TreeNode(subtree) for subtree in children_yaml]
        ...
        def verify_probabilities(self):
            ...
            if abs(cumulative - 1) > 1e-6:
                raise TrajectoryTreeModule.InvalidTreeStructure(
                    f"Sum of transition probabilities to children for node {self.name} is not 1 : got {cumulative}"
                )
            for child in self.children:
                child.verify_probabilities()
    ...
    class TrajectoryNodeData:
        ...
        def __init__(self, name : str, yaml_data, studies_pathes : dict[str, Path]):
            self.name : str = name
            self.investment_date : int = yaml_data.get(TrajectoryInputKeys.investment_date_key(), 0)
    ...
    class CandidatesTypeCosts:
        ...
        def __init__(self, name : str, yaml_data):
            ...
            self.operation_maintenance = yaml_data.get(TrajectoryInputKeys.oandm_cost_key())
    ...
    class TrajectoryConstraints:
        ...
        def __init__(self, yaml_data, all_nodes : List[str], all_candidates : List[str]):
            ...
            # Candidates
            candidates_data = yaml_data.get(TrajectoryInputKeys.constraints_candidates_key())
            ...
            if candidates_data == TrajectoryInputKeys.constraint_all_keyword():
                raise NotImplementedError("'all'keyword for candidates not yet implemented")
            else:
        ...
        @staticmethod
        def build_variable_reference(node : str, candidate : str, variable_type : InvestmentVariableType):
            ...
            for node in self.nodes:
                for candidate in self.candidates:
                    ...
                    constraint[TrajectoryOuputKeys.constraint_coeffs_key()] = {
                        self.build_variable_reference(node, candidate, InvestmentVariableType.DX_PLUS) : 1
                    }
                    constraint[TrajectoryOuputKeys.constraint_rhs_key()] = self.rhs
                    constraint[TrajectoryOuputKeys.constraint_operator_key()] = ConstraintOperator.LEQ.value
        ...
        def to_individual_max_retirement(self) -> List[dict[str, any]]:
            ...
            for node in self.nodes:
                for candidate in self.candidates:
                    constraint = {}
                    constraint[TrajectoryOuputKeys.constraint_coeffs_key()] = {
                        self.build_variable_reference(node, candidate, InvestmentVariableType.DX_MINUS) : 1
                    }
                    constraint[TrajectoryOuputKeys.constraint_rhs_key()] = self.rhs
                    constraint[TrajectoryOuputKeys.constraint_operator_key()] = ConstraintOperator.LEQ.value
                    output.append(constraint)
        ...
        def to_cumulative_max_investment(self) -> List[dict[str, any]]:
            ...
            for node in self.nodes:
                ...
                constraint[TrajectoryOuputKeys.constraint_operator_key()] = ConstraintOperator.LEQ.value
                for candidate in self.candidates:
                    ref = self.build_variable_reference(node, candidate, InvestmentVariableType.DX_PLUS)
    ...
    def parse_trajectory_user_file(self):
        ...
        with open(self.input_file) as file:
            ...
            capacities_raw_data = content.get(TrajectoryInputKeys.initial_capacities_key())
            ...
            self.global_data = self.TrajectoryGlobalData(global_raw_data, capacities_raw_data)
            ...
            if len(tree_data) > 1:
                raise self.InvalidTreeStructure("There can only be one root node to the tree")
            ...
            candidates_types_data = content.get(TrajectoryInputKeys.candidates_types_key())
            if nodes_data is None:
                raise self.InvalidInputFile(
    ...
    def write_merger_json(self, output_file : Path):
        ...
        for constraint in self.constraints:
            output[TrajectoryOuputKeys.constraint_key()].extend(constraint.to_merger_json())
...
parser = TrajectoryTreeModule("merge_master_test/simple_tree/user_input_XpansionTrajectory.yaml")
