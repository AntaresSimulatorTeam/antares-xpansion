...
class XpansionDriver:
    ...
    def __init__(self, config_loader: ConfigLoader):
        ...
        self.antares_driver = AntaresDriver(self.config_loader.antares_exe())
        data = ProblemGeneratorData(
            keep_mps=self.config_loader.keep_mps(),
    ...
    def clean_step(self):
        ret = subprocess.run(
            [
                str(self.config_loader.antares_archive_updater_exe()),
                "-a",
                str(self.config_loader.simulation_output_path()),
                "-p",
                self.config_loader.simulation_lp_path(),
                self.config_loader.expansion_dir(),
                "-d",
            ],
            shell=False,
            stdout=sys.stdout,
    ...
    def launch_antares_step(self):
        ...
        try:
            ret = self.antares_driver.launch(
                self.config_loader.data_dir(),
                self.config_loader.antares_n_cpu(),
                use_xpress,
            )
            if ret is False:
    ...
    def launch_problem_generation_step_memory(self):
        self.problem_generator_driver.launch_memory(
