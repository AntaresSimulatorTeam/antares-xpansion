    ...
    logFilePath_(logFilePath)
{
    ...
    if (logFile_.fail())
    {
        std::cerr << "ProblemGenerationFileLogger: Invalid file (" << logPath.string()
...
ProblemGenerationOstreamLogger::ProblemGenerationOstreamLogger(std::ostream& stream):
    stream_(stream)
{
    SetType(LogUtils::LOGGERTYPE::CONSOLE);
    if (stream_.fail())
    {
        std::cerr << "ProblemGenerationOstreamLogger: Invalid stream passed as parameter"
                  << std::endl;
    }
}

void ProblemGenerationOstreamLogger::display_message(const std::string& message)
{
...
std::shared_ptr<ProblemGenerationLogger> BuildLogger(const std::filesystem::path& log_file_path,
                                                     std::ostream& stream,
                                                     const std::string& context)
{
