...
using ProblemGenerationILoggerSharedPointer = std::shared_ptr<ProblemGenerationILogger>;
...
public:
    ~ProblemGenerationFileLogger() override
    {
        logFile_.close();
    }
...
public:
    ...
    ProblemGenerationLogger& operator<<(const std::shared_ptr<ProblemGenerationLogger> logger)
    {
        return (*logger);
    }
...
template<typename T>
ProblemGenerationLogger& ProblemGenerationLogger::operator<<(const T& t)
{
