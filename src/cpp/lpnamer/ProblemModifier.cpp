...
void ProblemModifier::setProblem(const std::shared_ptr<SolverAbstract> &mathProblem) {
    _math_problem = mathProblem;
}

std::shared_ptr<SolverAbstract> ProblemModifier::changeProblem(std::shared_ptr<SolverAbstract> &mathProblem,
                                                               const ColumnsToChange &columns_to_change) {
    _math_problem = mathProblem;
...
void ProblemModifier::changeProblem(const ActiveLinks &active_links) {
    ...
    auto all_candidates = collect_candidates_from_all_links(active_links);
    add_new_columns(all_candidates);

}

Cands ProblemModifier::collect_candidates_from_all_links(const ActiveLinks &active_links) const {
