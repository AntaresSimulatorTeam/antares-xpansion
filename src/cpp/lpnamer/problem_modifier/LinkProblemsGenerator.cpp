...
{
    auto in_prblm = problem_provider->provide_problem(solver_config_.Name(), solver_log_manager_);
                                  ...
                                  IProblemWriter* writer)
{
    ...
    problem_modifier.changeProblem(problem,
                                   _links,
    ...
    for (const ActiveLink& link: _links)
    {
        for (const Candidate& candidate: link.getCandidates())
        {
            if (problem_modifier.has_candidate_col_id(candidate.get_name()))
            {
                ...
                couplings[{candidate.get_name(), problem_name}] = problem_modifier
                                                                    .get_candidate_col_id(
                                                                      candidate.get_name());
            }
        }
    }
    ...
    writer->Write_problem(&forWriting, lp_mps_name, true);
    (*logger_)(LogUtils::LOGLEVEL::INFO)
                                      ...
                                      IProblemWriter* writer)
{
    std::for_each(
      ...
      [&](const auto& mps)
      {
          auto adapter = std::make_unique<MPSFileProblemProviderAdapter>(root,
                                                                         mps._problem_filename);
          auto problem = adapter->provide_problem(solver_config_.Name(), solver_log_manager_);
          std::unique_ptr<IProblemVariablesProviderPort> variables_provider;
          if (rename_problems_)
          {
              variables_provider = std::make_unique<ProblemVariablesFileAdapter>(mps,
                                                                                 _links,
                                                                                 logger_,
                                                                                 root);
          }
          ...
          {
              variables_provider = std::make_unique<ProblemVariablesFromProblemAdapter>(problem,
                                                                                        _links,
                                                                                        logger_);
          }

          treat(mps._problem_filename, couplings, problem.get(), variables_provider.get(), writer);
      });
}
