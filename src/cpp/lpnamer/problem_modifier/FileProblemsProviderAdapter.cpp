...
std::vector<std::shared_ptr<Problem>> FileProblemsProviderAdapter::provideProblems(
  ...
  SolverLogManager& solver_log_manager) const
{
    ...
    std::transform(std::execution::par,
                   ...
                   [&](auto name)
                   {
                       ...
                       return problem_provider.provide_problem(solver_name, solver_log_manager);
                   });
    return problems;
}

FileProblemsProviderAdapter::FileProblemsProviderAdapter(std::filesystem::path lp_dir,
                                                         std::vector<std::string> problem_names):
    lp_dir_(lp_dir),
    problem_names_(problem_names)
{
}
