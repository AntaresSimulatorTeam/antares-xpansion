...
{
public:
    explicit FileProblemProviderAdapter(std::filesystem::path lp_dir, std::string problem_name);
    virtual ~FileProblemProviderAdapter() = default;
    std::shared_ptr<Problem> provide_problem(const std::string& solver_name,
