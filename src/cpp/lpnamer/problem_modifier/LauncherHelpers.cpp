  ...
  std::shared_ptr<ProblemGenerationLog::ProblemGenerationLogger> logger)
{
    ...
    addBinaryVariables(master_p, additionalConstraints_p.getVariablesToBinarise(), logger);
...
char getConstraintSenseSymbol(const AdditionalConstraint& additionalConstraint_p,
                              std::shared_ptr<ProblemGenerationLog::ProblemGenerationLogger> logger)
{
    char rtype;
    if (std::string sign_l = additionalConstraint_p.getSign(); sign_l == "less_or_equal")
    {
        rtype = 'L';
    }
    ...
    else
    {
        (*logger)(LogUtils::LOGLEVEL::FATAL)
          << LOGLOCATION << "FATAL: in addAdditionalConstraint, unknown row type " << sign_l
          << std::endl;
        std::exit(1);
    }
    return rtype;
                             ...
                             std::shared_ptr<ProblemGenerationLog::ProblemGenerationLogger> logger)
{
    ...
    int i = 0;
    for (const auto& [name, coeff]: additionalConstraint_p)
    {
        int col_index = master_p->get_col_index(name);
        if (col_index == -1)
        {
            (*logger)(LogUtils::LOGLEVEL::FATAL) << LOGLOCATION << "missing variable " << name
                        ...
                        std::shared_ptr<ProblemGenerationLog::ProblemGenerationLogger> logger)
{
    for (const auto& pairOldNewVarnames: variablesToBinarise_p)
    {
        ...
        master_p->add_cols(1,
                           ...
                           std::vector<int>(2, 0).data(),
                           std::vector<int>(0).data(),
                           std::vector<double>(0).data(),
                           std::vector<double>(1, 0).data(),
                           std::vector<double>(1, 1e20).data(),
                           {});
  ...
  std::shared_ptr<ProblemGenerationLog::ProblemGenerationLogger> logger)
{
    ...
    CandidatesINIReader candidateReader(interco_file_name, area_file_name, logger);

    // Get all mandatory path
    const auto xpansion_user_dir = xpansion_output_dir / ".." / ".." / "user" / "expansion";
    const auto candidates_file_name = xpansion_user_dir / CANDIDATES_INI;
    const auto capacity_folder = xpansion_user_dir / "capa";

    // Instantiation of candidates
    const auto& candidatesDatas = candidateReader.readCandidateData(candidates_file_name);
    const auto& mapLinkProfile = LinkProfileReader(logger).getLinkProfileMap(capacity_folder,
                                                                             candidatesDatas);

    return ActiveLinksBuilder(candidatesDatas,
                              mapLinkProfile,
                              DirectAccessScenarioToChronicleProvider(ts_root, logger),
                              logger);
}
