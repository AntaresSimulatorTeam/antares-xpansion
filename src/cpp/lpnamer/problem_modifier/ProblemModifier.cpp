...
std::vector<int> extract_col_ids(const ColumnsToChange &columns_to_change)
{
    ...
    std::transform(columns_to_change.begin(), columns_to_change.end(), std::back_inserter(col_ids), [](const ColumnToChange &col)
                   { return col.id; });
    return col_ids;
}
...
SolverAbstract::Ptr
ProblemModifier::changeProblem(SolverAbstract::Ptr mathProblem, const std::vector<ActiveLink> &active_links,
