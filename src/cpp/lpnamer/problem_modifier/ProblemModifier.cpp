...
std::set<int> extract_time_steps(const std::map<linkId, ColumnsToChange>& p_columns)
{
    std::set<int> result;
  ...
  const std::set<int>& time_steps) const
{
    ...
    {
        const auto& candidates = link.getCandidates();
...
std::vector<int> extract_col_ids(const ColumnsToChange& columns_to_change)
{
    ...
    std::transform(columns_to_change.begin(),
                   columns_to_change.end(),
  ...
  const std::map<linkId, ColumnsToChange>& p_indirect_cost_columns)
{
    ...
    add_new_columns(
      candidates_with_not_null_profile(active_links, extract_time_steps(p_ntc_columns)));
  ...
  const std::map<linkId, ColumnsToChange>& p_ntc_columns)
{
    ...
    for (const auto& link: active_links)
    {
        ...
        {
            add_direct_profile_column_constraint(dmatval,
                                                 colind,
                                                 rowtype,
                                                 rhs,
                                                 rstart,
                                                 link,
                                                 column);
            add_indirect_profile_ntc_column_constraint(dmatval,
                                                       colind,
                                                       rowtype,
                                                       rhs,
                                                       rstart,
  ...
  const std::map<linkId, ColumnsToChange>& p_cost_columns)
{
    ...
    for (const auto& link: active_links)
    {
        if (p_cost_columns.find(link.get_idLink()) != p_cost_columns.end())
        {
            for (const ColumnToChange& column: p_cost_columns.at(link.get_idLink()))
            {
                add_indirect_cost_column_constraint(dmatval,
                                                    colind,
                                                    rowtype,
                                                    rhs,
                                                    rstart,
...
void ProblemModifier::add_direct_profile_column_constraint(std::vector<double>& dmatval,
                                                           std::vector<int>& colind,
                                                           std::vector<char>& rowtype,
                                                           std::vector<double>& rhs,
                                                           std::vector<int>& rstart,
                                                           ...
                                                           const ColumnToChange& column)
{
    ...
    double direct_already_installed_profile_at_timestep = link.already_installed_direct_profile(
      chronicle_to_use,
      column.time_step);
    ...
    for (const auto& candidate: link.getCandidates())
    {
        ...
        {
            colind.push_back(static_cast<int>(_candidate_col_id[candidate.get_name()]));
...
void ProblemModifier::add_indirect_profile_ntc_column_constraint(std::vector<double>& dmatval,
                                                                 std::vector<int>& colind,
                                                                 std::vector<char>& rowtype,
                                                                 std::vector<double>& rhs,
                                                                 std::vector<int>& rstart,
...
bool ProblemModifier::candidateContributionIndirectIsNotNull(const ColumnToChange& column,
                                                             unsigned int chronicle_to_use,
                                                             const Candidate& candidate) const
{
...
void ProblemModifier::add_indirect_cost_column_constraint(std::vector<double>& dmatval,
                                                          std::vector<int>& colind,
                                                          std::vector<char>& rowtype,
                                                          std::vector<double>& rhs,
                                                          std::vector<int>& rstart,
...
void ProblemModifier::add_new_columns(const std::vector<Candidate>& candidates)
{
    if (!candidates.empty())
    {
        ...
        for (const auto& candidate: candidates)
        {
            ...
            unsigned int new_index = static_cast<unsigned>(_candidate_col_id.size())
                                     + _n_cols_at_start;
            _candidate_col_id[candidate.get_name()] = new_index;
        }
        solver_addcols(*_math_problem,
                       objectives,
                       mstart,
                       {},
                       {},
