  ...
  std::map<colId, ColumnsToChange>& p_indirect_cost_columns) const
{
    ...
    auto variableReader = VariableFileReader(variableFileContent,
                                             links_,
...
{
    auto variableFileContent = archive_reader_->ExtractFileInStringStream(
      problem_data_._variables_txt);

    ProblemVariables result;
    extract_variables(variableFileContent,
                      result.variable_names,
                      result.ntc_columns,
                      result.direct_cost_columns,
