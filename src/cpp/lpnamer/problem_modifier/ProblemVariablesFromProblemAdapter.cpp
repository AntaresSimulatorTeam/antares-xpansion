...
void ReadLinkZones(const std::string& input, std::string& origin, std::string& destination)
{
    // input format should be link<area1$$area2>
    const auto zones = StringManip::split(StringBetweenChevrons(input), AREA_SEPARATOR);
    origin = zones[0];
...
void updateMapColumn(const std::vector<ActiveLink>& links,
                     ...
                     const std::string_view link_destination,
                     colId id,
                     int time_step,
...
ProblemVariables ProblemVariablesFromProblemAdapter::Provide()
{
    ...
    extract_variables(result.ntc_columns, result.direct_cost_columns, result.indirect_cost_columns);
    return result;
}

ProblemVariablesFromProblemAdapter::ProblemVariablesFromProblemAdapter(
  std::shared_ptr<Problem> problem,
  std::vector<ActiveLink> vector_1,
