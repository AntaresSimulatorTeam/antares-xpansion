...
void AdditionalConstraints::ReadConstraintsFile()
{
    ...
    const std::map<std::string, std::string>& variables_section = reader_l.getVariablesSection();
    ...
    for (const std::string& sectionName_l: reader_l.getSections())
    {
        ...
        {
            const std::map<std::string, std::string>& constarintsSection_l = reader_l.getSection(
              sectionName_l);
...
void AdditionalConstraints::addVariablesToBinarise(
  const std::map<std::string, std::string>& variables_section)
{
...
const std::map<std::string, std::string>& AdditionalConstraints::getVariablesToBinarise() const
{
  ...
  const std::map<std::string, std::string>& constarintsSection_l)
{
    ...
    std::string constraintName_l = checkAndReturnConstraintName(sectionName_l,
                                                                constarintsSection_l);
    // check that section has defined a sign
    std::string constraintSign_l = checkAndReturnSectionSign(sectionName_l, constarintsSection_l);
    // check that section has defined a rhs
    double constraintRHS_l = checkAndReturnSectionRhs(sectionName_l, constarintsSection_l);
    ...
    {
        if (pairAttributeValue_l.first == "name" || pairAttributeValue_l.first == "rhs"
        ...
        else
        {
            ...
            catch (const std::invalid_argument& e)
            {
                (*logger_)(LogUtils::LOGLEVEL::FATAL)
                  ...
                  << " in section " << sectionName_l << ": coeff value must be a double!\n";
                std::exit(1);
            }
        }
    }
    ...
    {
        (*logger_)(LogUtils::LOGLEVEL::FATAL)
          << LOGLOCATION << "section " << sectionName_l << " defines an empty constraint.\n";
        std::exit(1);
    }
  ...
  const std::map<std::string, std::string>& constarintsSection_l) const
{
    ...
    {
        (*logger_)(LogUtils::LOGLEVEL::FATAL)
          << LOGLOCATION << "section " << sectionName_l << " is missing a name.\n";
        std::exit(1);
    }
    else
    {
  ...
  const std::map<std::string, std::string>& constarintsSection_l) const
{
    ...
    {
        (*logger_)(LogUtils::LOGLEVEL::FATAL)
          << LOGLOCATION << "section " << sectionName_l << " is missing a sign.\n";
        std::exit(1);
    }
    else
    {
  ...
  const std::map<std::string, std::string>& constarintsSection_l) const
{
    ...
    else
    {
        ...
        {
            (*logger_)(LogUtils::LOGLEVEL::FATAL)
              << LOGLOCATION << "Invalid value " << temporatyIterator_l->second << " in section "
