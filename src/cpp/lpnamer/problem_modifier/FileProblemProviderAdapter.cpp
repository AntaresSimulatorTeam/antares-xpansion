  ...
  SolverLogManager& solver_log_manager) const
{
    ...
    const std::filesystem::path problem_path = lp_dir_.parent_path() / problem_name_;
    in_prblm->read_prob_mps(problem_path);
    std::filesystem::remove(problem_path);
    return in_prblm;
}

FileProblemProviderAdapter::FileProblemProviderAdapter(std::filesystem::path lp_dir,
