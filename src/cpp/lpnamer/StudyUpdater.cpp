...
std::pair<double, double> StudyUpdater::computeNewCapacities(std::vector<double> investments_p, ActiveLink& link_p, int timepoint_p) const
{
    ...
    for (int index_candidate = 0; index_candidate < candidates.size(); index_candidate++)
    {
        const auto& candidate = candidates.at(index_candidate);

        direct_l    += investments_p.at(index_candidate) * candidate.direct_profile(timepoint_p);
        indirect_l  += investments_p.at(index_candidate) * candidate.indirect_profile(timepoint_p);
    }
    return std::make_pair(direct_l, indirect_l);
}
...
int StudyUpdater::updateLinkdataFile(ActiveLink link_p, std::vector<double> investment_from_candidates_p) const
{
    ...
    for (int line_cnt = 0; line_cnt < 8760; ++line_cnt)
    {
        ...
        std::pair<double, double> newCapacities_l = computeNewCapacities(investment_from_candidates_p, link_p, line_cnt);
        record_l.updateCapacities(newCapacities_l.first, newCapacities_l.second);
        tempOutCsvFile << record_l.to_row("\t") << "\n";
    }
...
int StudyUpdater::update(std::vector<ActiveLink> const& links_p, std::map<std::string, double> investments_p) const
{
    ...
    {
        const auto& candidatesFromLink = link.getCandidates();
        std::vector<double> investments_from_link_candidates;
        investments_from_link_candidates.reserve(candidatesFromLink.size());
        for (const auto& candidate : candidatesFromLink)
        {
            investments_from_link_candidates.push_back(investments_p[candidate._name]);
        }
        updateFailures_l += updateLinkdataFile(link, investments_from_link_candidates);
    }
