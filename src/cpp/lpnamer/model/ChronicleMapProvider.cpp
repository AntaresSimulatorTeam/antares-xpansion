...
std::map<unsigned int, unsigned int> DirectAccessScenarioToChronicleProvider::GetMap(
  ...
  const std::string& link_to) const
{
    ...
    if (!file.is_open())
    {
        (*logger_)(LogUtils::LOGLEVEL::INFO)
          << "No scenario builder output found for link at destination: " << file_path << std::endl
