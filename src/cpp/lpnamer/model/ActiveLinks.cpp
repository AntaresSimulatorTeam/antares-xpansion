...
void ActiveLinksBuilder::addCandidate(const CandidateData& candidate_data)
{
    ...
    _links.at(indexLink).addCandidate(candidate_data,
                                      getProfilesFromProfileMap(
...
ActiveLinksBuilder::ActiveLinksBuilder(
  ...
  std::shared_ptr<ProblemGenerationLog::ProblemGenerationLogger> logger):
    ActiveLinksBuilder(candidateList,
                       profile_map,
...
void ActiveLinksBuilder::checkLinksValidity()
{
    for (const auto& candidateData: _candidateDatas)
    {
        ...
        launchExceptionIfNoLinkProfileAssociated(candidateData.installed_direct_link_profile_name);
  ...
  const LinkName& link_name) const
{
    ...
    if (doubles_are_different(link_data.installed_capacity, old_link_data.installed_capacity))
    {
        ...
        auto message = "Multiple already installed capacity detected for link " + link_name;
        (*logger_)(LogUtils::LOGLEVEL::FATAL) << log_location << message;
        throw MultipleAlreadyInstalledCapacityDetectedForLink(message, log_location);
    }
    if (old_link_data.profile_name != link_data.profile_name)
    {
        auto log_location = LOGLOCATION;
        auto message = "Multiple already_installed_profile detected for link " + link_name;
        (*logger_)(LogUtils::LOGLEVEL::FATAL) << log_location << message;
  ...
  const std::string& profileName) const
{
    if (!profileName.empty())
    {
        ...
        if (it_profile == _profile_map.end())
        {
            ...
            (*logger_)(LogUtils::LOGLEVEL::FATAL) << log_location << message;
            throw ThereIsNoLinkProfileAssociatedWithThisProfile(message, log_location);
        }
    }
}
...
void ActiveLinksBuilder::checkCandidateNameDuplication() const
{
    ...
    for (const auto& candidateData: _candidateDatas)
    {
        ...
        {
            auto log_location = LOGLOCATION;
            auto message = "Candidate " + candidateData.name + " duplication detected";
            (*logger_)(LogUtils::LOGLEVEL::FATAL) << log_location << message;
...
{
    for (const auto& [link_name, link_data]: _links_data)
    {
        auto mc_year_to_chronicle = scenario_to_chronicle_provider_.GetMap(link_data._linkor,
                                                                           link_data._linkex);
        ActiveLink link(link_data.id,
                        link_name,
                        link_data._linkor,
                        link_data._linkex,
                        link_data.installed_capacity,
                        mc_year_to_chronicle,
                        logger_);
        link.setAlreadyInstalledLinkProfiles(getProfilesFromProfileMap(link_data.profile_name));
  ...
  const std::string& profile_name) const
{
    ...
    if (profiles.empty())
    {
        return {LinkProfile(logger_)};
    }
    return profiles;
}

ActiveLink::ActiveLink(int idLink,
                       std::string linkName,
                       std::string linkor,
                       std::string linkex,
...
ActiveLink::ActiveLink(int idLink,
                       ...
                       const double& already_installed_capacity,
                       std::shared_ptr<ProblemGenerationLog::ProblemGenerationLogger> logger):
    ActiveLink(idLink, linkName, linkor, linkex, already_installed_capacity, {}, logger)
{
}

void ActiveLink::setAlreadyInstalledLinkProfiles(const std::vector<LinkProfile>& linkProfile)
{
...
unsigned ActiveLink::get_idLink() const
{
    return _idLink;
}
...
const std::string& ActiveLink::linkor() const
{
    return _linkor;
}
...
const std::string& ActiveLink::linkex() const
{
    return _linkex;
}
