...
public:
     ...
     *
     */
    explicit LinkProfile(std::shared_ptr<ProblemGenerationLog::ProblemGenerationLogger> logger):
        logger_(logger)
    {
    }
    ...
    // ADR
    std::vector<double> direct_link_profile = std::vector<double>(NUMBER_OF_HOUR_PER_YEAR, 1);
    //! indirect linkprofile values if different from direct linkprofile
    std::vector<double> indirect_link_profile = std::vector<double>(NUMBER_OF_HOUR_PER_YEAR, 1);
    ...
    class InvalidHourForProfile: public LogUtils::XpansionError<std::invalid_argument>
    {
