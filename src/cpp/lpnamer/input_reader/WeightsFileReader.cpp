...
bool WeightsFileReader::CheckWeightsFile()
{
    ...
    if (!file_reader)
    {
        ...
        msg << "Could not open weights file: " << weights_file_path_.string() << std::endl;
    ...
    if (nb_values != number_of_active_years_)
    {
        std::ostringstream msg;
        auto log_location = LOGLOCATION;
        msg << " file " << weights_file_path_.string() << " : invalid weight number : " << nb_values
            << " values / " << number_of_active_years_ << " expected" << std::endl;
    ...
    if (null_weights)
    {
        std::ostringstream msg;
        auto log_location = LOGLOCATION;
        msg << "file " << weights_file_path_.string() << ": all values are null" << std::endl;
        (*logger_)(LogUtils::LOGLEVEL::FATAL) << log_location << msg.str();
...
double WeightsFileReader::GetWeightFromLine(const std::string& line, int idx) const
{
    double weight;
    try
    {
...
bool WeightsFileReader::AreAllWeightsNull() const
{
    return null_weights;
}
...
void WeightsFileReader::CheckFileIsNotEmpty(std::ifstream& file) const
{
    if (file.peek() == std::ifstream::traits_type::eof())
    {
        std::ostringstream msg;
        auto log_location = LOGLOCATION;
        msg << "Error! Weights file is empty: " << weights_file_path_.string() << std::endl;
        (*logger_)(LogUtils::LOGLEVEL::FATAL) << log_location << msg.str();
