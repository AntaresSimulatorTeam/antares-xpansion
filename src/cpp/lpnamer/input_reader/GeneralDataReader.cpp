    ...
    logger_(logger)
{
    if (file_path.empty() || !std::filesystem::exists(file_path))
    {
        ...
        msg << LOGLOCATION << "General data file is not found : " << file_path.string()
            << std::endl;
...
std::vector<int> GeneralDataIniReader::ActiveYearsFromActiveList()
{
    ...
    {
        if (std::ranges::find(active_year_list_, year) != active_year_list_.end())
        {
...
std::vector<int> GeneralDataIniReader::ActiveYearsFromInactiveList()
{
    ...
    for (auto year = 0; year < mc_years_; year++)
    {
        if (std::ranges::find(inactive_year_list_, year) == inactive_year_list_.end())
        {
            active_years.push_back(year + 1);
        }
    }
...
std::string GeneralDataIniReader::ReadPlaylist(const std::string& current_section,
