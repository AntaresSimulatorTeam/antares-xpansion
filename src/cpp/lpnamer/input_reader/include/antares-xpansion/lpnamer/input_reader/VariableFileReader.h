...
{
public:
    VariableFileReader(const std::filesystem::path& fileName,
                       const std::vector<ActiveLink>& links,
                       const VariableFileReadNameConfiguration& variable_name_config,
                       std::shared_ptr<ProblemGenerationLog::ProblemGenerationLogger> logger);
    VariableFileReader(std::istringstream& fileInIStringStream,
                       const std::vector<ActiveLink>& links,
                       const VariableFileReadNameConfiguration& variable_name_config,
                       std::shared_ptr<ProblemGenerationLog::ProblemGenerationLogger> logger);
    void ReadVarsFromStream(std::istream& stream,
                            const std::vector<ActiveLink>& links,
                            const VariableFileReadNameConfiguration& variable_name_config);
