...
public:
    explicit LpFilesExtractor(std::filesystem::path antares_archive_path,
...
private:
    using areaAndIntercoPaths = std::pair<std::vector<std::filesystem::path>,
    ...
    void produceIntercotxtFile(const std::vector<std::filesystem::path>& vect_interco_files) const;
};
