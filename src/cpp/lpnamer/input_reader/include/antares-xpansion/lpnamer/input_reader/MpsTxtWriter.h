...
using MpsVariableConstraintsFiles = std::
  tuple<std::filesystem::path, std::filesystem::path, std::filesystem::path>;
...
struct ProblemData
{
    ProblemData(const std::string& problem_filename, const std::string& variables_txt):
        _problem_filename(problem_filename),
        _variables_txt(variables_txt)
    {
    }
...
private:
    ...
    void FillMapWithVariablesFiles(const std::vector<std::filesystem::path>& variables_files);
    void FillMapWithConstraintsFiles(const std::vector<std::filesystem::path>& variables_files);
    YearAndWeek YearAndWeekFromFileName(const std::filesystem::path& file_name) const;
    void FillMapFiles();
    void FillMapZip();
};
