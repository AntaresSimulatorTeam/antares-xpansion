...
void LinkProfileReader::EnsureFileIsGood(const std::filesystem::path& direct_filename) const
{
                                        ...
                                        bool fillDirectProfile)
{
    ...
    for (size_t time_step(0); time_step < NUMBER_OF_HOUR_PER_YEAR; ++time_step)
    {
        if (std::getline(infile, line))
        {
            ...
            while (buffer >> str_value)
            {
                if (is_number(str_value, value))
                {
                    ...
                    UpdateProfile(result, fillDirectProfile, value, chronicle_id, time_step);
                    ++chronicle_id;
                }
                else
                {
                    auto errMsg = std::string("Error while reading value in link-profile ")
        ...
        {
            auto errMsg = std::string("error not enough line in link-profile ") + filename.string();
            (*logger_)(LogUtils::LOGLEVEL::FATAL) << LOGLOCATION << errMsg;
            throw std::domain_error(errMsg);
        }
    }
...
void LinkProfileReader::ConstructChronicle(std::vector<LinkProfile>& result, int chronicle_id) const
{
