...
void AdditionalConstraintsReader::processEntryLine()
{
    ...
    size_t illegalCharIndex_l = attribute_l.find_first_of(
      AdditionalConstraintsReader::illegal_chars);
    ...
    else
    {
        if ((attribute_l == "name") || (_section == "variables"))
        {
            ...
            if (illegalCharIndex_l != std::string::npos)
            {
                (*logger_)(LogUtils::LOGLEVEL::FATAL)
                  << LOGLOCATION << "additional constraints file: line " << _lineNb
                  << " : Illegal character '" << value_l[illegalCharIndex_l] << "' in value!\n";
                std::exit(1);
            }
        }
...
const std::map<std::string, std::string>& AdditionalConstraintsReader::getVariablesSection()
{
...
const std::map<std::string, std::string>& AdditionalConstraintsReader::getSection(
  const std::string& sectionName_p) const
{
