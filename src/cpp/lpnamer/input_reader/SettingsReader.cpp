...
void SettingsReader::parse_file(const std::filesystem::path& file_path)
{
    ...
    {
        if (std::find_if(line.begin(), line.end(), [](auto c) { return !std::isspace(c); })
            == line.end())
        {
        ...
        if (values.size() != 2)
        {
            ...
            msg << "Invalid line in settings file : " << line << std::endl;
...
void SettingsReader::check_file_exist(const std::filesystem::path& file_path) const
{
    if (file_path.empty() || !std::filesystem::exists(file_path))
    {
        std::ostringstream msg;
        msg << LOGLOCATION << "Settings file is not found : " << file_path.string() << std::endl;
        (*this->logger_)(LogUtils::LOGLEVEL::FATAL) << msg.str();
