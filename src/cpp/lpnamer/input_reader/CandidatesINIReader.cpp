    ...
    logger_(logger)
{
    ...
    for (const auto& intercoFileData: _intercoFileData)
    {
        // TODO : check if index is available in areaNames
        const std::string& pays_or(_areaNames[intercoFileData.index_pays_origine]);
        const std::string& pays_ex(_areaNames[intercoFileData.index_pays_extremite]);
        std::string linkName = pays_or + " - " + pays_ex;
        _intercoIndexMap[linkName] = intercoFileData.index_interco;
    }
}

void CandidatesINIReader::ProcessAreaFile(const std::filesystem::path& areaFile)
{
    ...
    if (const auto& msg = areaFileData.error_message; !msg.empty())
    {
        ...
        throw AreaFileError(PrefixMessage(LogUtils::LOGLEVEL::FATAL, logger_->getContext()), msg);
    }
...
std::vector<IntercoFileData> CandidatesINIReader::ReadLineByLineInterco(std::istream& stream) const
{
...
std::string getStrVal(const INIReader& reader,
...
bool CandidatesINIReader::checkArea(const std::string& areaName_p) const
{
    bool found_l = std::find(_areaNames.cbegin(), _areaNames.cend(), areaName_p)
                   != _areaNames.cend();
    return found_l;
}
  ...
  const std::filesystem::path& candidateFile) const
{
    ...
    {
        CandidateData candidateData = readCandidateSection(candidateFile,
                                                           reader,
                                                           sectionName,
                                                           err_msg);
...
CandidateData CandidatesINIReader::readCandidateSection(const std::filesystem::path& candidateFile,
                                                        ...
                                                        std::ostringstream& err_msg) const
{
    ...
    if (i != std::string::npos)
    {
        ...
        candidateData.linkex = candidateData.link_name.substr(i + 3,
                                                              candidateData.link_name.size());
        if (!checkArea(candidateData.linkor))
        {
            err_msg << LOGLOCATION << "Unrecognized area " << candidateData.linkor << " in section "
                    << sectionName << " in " << candidateFile.string() << "\n";
        }
        if (!checkArea(candidateData.linkex))
        {
            err_msg << LOGLOCATION << "Unrecognized area " << candidateData.linkex << " in section "
                    << sectionName << " in " << candidateFile.string() << "\n";
        }
    }
    ...
    {
        err_msg << LOGLOCATION << "cannot link candidate " << candidateData.name << " to interco id"
                << "\n";
    }
    ...
    candidateData.installed_direct_link_profile_name = getStrVal(
      reader,
      sectionName,
      "already-installed-direct-link-profile");
    candidateData.installed_indirect_link_profile_name = getStrVal(
      reader,
      sectionName,
    ...
    candidateData.already_installed_capacity = getDblVal(reader,
                                                         sectionName,
