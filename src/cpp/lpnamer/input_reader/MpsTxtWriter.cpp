...
auto FileWithExtension(std::string_view ext, const std::filesystem::path& directory)
{
...
void FilesMapper::FillMapFiles()
{
    ...
    for (const auto& file: vect_of_files_with_txt_ext)
    {
        ...
        auto is_it_variables_file = filename_str.find("variables"s) != std::string::npos;
        auto is_it_constraints_file = filename_str.find("constraints"s) != std::string::npos;
...
void FilesMapper::FillMapZip()
{
    ...
    std::vector<std::filesystem::path> variables_files;
    std::vector<std::filesystem::path> constraints_files;
    auto vect_of_files_with_txt_ext = zip_reader.GetEntriesPathWithExtension(".txt"s);
    for (const auto& file: vect_of_files_with_txt_ext)
    {
        auto filename_str = file.stem().string();
        auto is_it_variables_file = filename_str.find("variables"s) != std::string::npos;
...
void FilesMapper::FillMapWithMpsFiles(const std::vector<std::filesystem::path>& mps_files)
{
    for (const auto& mps_file: mps_files)
    {
        year_week_and_files_dict_.try_emplace(YearAndWeekFromFileName(mps_file), mps_file, "", "");
    }
}
  ...
  const std::vector<std::filesystem::path>& variables_files)
{
    for (const auto& variables_file: variables_files)
    {
        ...
        auto& [dummy1, variables_file_field, dummy2] = year_week_and_files_dict_[year_and_week];
  ...
  const std::vector<std::filesystem::path>& constraints_files)
{
    for (const auto& constraints_file: constraints_files)
    {
        ...
        auto& [dummy1, dummy2, constraints_file_field] = year_week_and_files_dict_[year_and_week];
        constraints_file_field = constraints_file;
    }
}

YearAndWeek FilesMapper::YearAndWeekFromFileName(const std::filesystem::path& file_name) const
{
    auto split_file_name = StringManip::split(StringManip::trim(file_name.filename().string()),
                                              '-');
...
std::vector<ProblemData> FilesMapper::MpsAndVariablesFilesVect()
{
    ...
    FillMap();
    std::transform(year_week_and_files_dict_.cbegin(),
                   year_week_and_files_dict_.cend(),
                   std::back_inserter(problems_data_vect),
                   [](const std::pair<YearAndWeek, MpsVariableConstraintsFiles>& year_week_files)
                   {
