 ...
 *
 * @return A pair of vectors containing the paths of the files. The first vector contains the paths
 * of the "area*.txt" files and the second vector contains the paths of the "interco*.txt" files.
 */
LpFilesExtractor::areaAndIntercoPaths LpFilesExtractor::getFiles() const
{
    ...
    case SimulationInputMode::FILE:
    {
        ...
        std::ranges::for_each(dit,
                              [&vect_area_files, &vect_interco_files](const auto& entry)
                              {
    ...
    break;
    default:
        throw LogUtils::XpansionError<std::runtime_error>("SimulationInputMode is not supported:",
                                                          LOGLOCATION);
    }
 ...
 *
 * The patterns it looks for are "area*.txt", "interco*.txt", and "ts-numbers*". The extracted files
 * are stored in the `xpansion_output_dir_` directory.
 *
 * @return A pair of vectors containing the paths of the extracted files. The first vector contains
 * the paths of the "area*.txt" files and the second vector contains the paths of the "interco*.txt"
 * files.
 */
LpFilesExtractor::areaAndIntercoPaths LpFilesExtractor::getFilesFromArchive() const
{
    ...
    archive_reader.Open();
    vect_area_files = archive_reader.ExtractPattern("area*.txt", "", this->xpansion_output_dir_);
  ...
  const std::vector<std::filesystem::path>& vect_area_files) const
{
    ...
    case SimulationInputMode::FILE:
        try
        {
            std::filesystem::rename(vect_area_files[0], this->xpansion_output_dir_ / "area.txt");
        }
...
{
    if (auto num_interco_file = vect_interco_files.size(); num_interco_file == 0)
    {
        std::ostringstream msg;
  ...
  const std::vector<std::filesystem::path>& vect_interco_files) const
{
    ...
    case SimulationInputMode::FILE:
        ...
        break;
    case SimulationInputMode::ARCHIVE:
        std::filesystem::rename(vect_interco_files[0], this->xpansion_output_dir_ / "interco.txt");
        break;
    default:
        throw LogUtils::XpansionError<std::runtime_error>("SimulationInputMode is not supported:",
                                                          LOGLOCATION);
