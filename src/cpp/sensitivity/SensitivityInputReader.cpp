...
Json::Value read_json(const std::filesystem::path& json_file_path)
{
    ...
    else
    {
        throw std::runtime_error(
          fmt::format("{}: unable to open : {}", LOGLOCATION, json_file_path.string()));
    }
    return json_data;
}

SensitivityInputReader::SensitivityInputReader(const std::filesystem::path& json_input_path,
                                               const std::filesystem::path& benders_output_path,
                                               std::filesystem::path last_master_path,
                                               std::filesystem::path basis_path,
...
std::shared_ptr<SolverAbstract> SensitivityInputReader::get_last_master() const
{
    ...
    case ProblemsFormat::SAVED_FILE:
        ...
        break;
    default:
        throw std::runtime_error(fmt::format("{}: unsupported format : {}", LOGLOCATION, format));
    }
...
std::map<std::string, std::pair<double, double>> SensitivityInputReader::get_candidates_bounds(
  ...
  const std::map<std::string, int>& name_to_id) const
{
    ...
    std::vector<std::string> candidates_name = last_master->get_col_names(0, nb_candidates - 1);
...
{
    return _benders_data[Output::SOLUTION_C][Output::INVESTMENT_COST_C].asDouble();
}

std::map<std::string, double> SensitivityInputReader::get_benders_solution() const
{
    ...
    {
        investment_solution[candidate_name] = _benders_data[Output::SOLUTION_C][Output::VALUES_C]
                                                           [candidate_name]
                                                             .asDouble();
    }
...
std::map<std::string, int> SensitivityInputReader::get_name_to_id() const
{
    ...
    {
        throw std::runtime_error(LOGLOCATION + "unable to open : " + _structure_file_path.string());
    }
    return name_to_id;
}
...
SensitivityInputData SensitivityInputReader::get_input_data() const
{
    ...
    input_data.candidates_bounds = get_candidates_bounds(input_data.last_master.get(),
                                                         input_data.name_to_id);
