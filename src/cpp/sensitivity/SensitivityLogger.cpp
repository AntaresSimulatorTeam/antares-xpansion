...
{
    _stream << std::endl;
    _stream << "--- Recall of the best investment solution ---" << std::endl << std::endl;
...
void SensitivityLogger::log_benders_overall_cost(const double& best_ub)
{
    _stream << indent_1 << "Best overall cost = "
            << xpansion::logger::commons::create_str_million_euros(best_ub) << MILLON_EUROS
            << std::endl
            << std::endl;
}
...
{
    _stream << indent_1
            << "Best capex = " << xpansion::logger::commons::create_str_million_euros(benders_capex)
            << MILLON_EUROS << std::endl
            << std::endl;
}

void SensitivityLogger::log_benders_solution(const std::map<std::string, double>& benders_solution)
{
    ...
    for (const auto& [candidate_name, investment]: benders_solution)
    {
        _stream << indent_1 << indent_1 << candidate_name << ": "
                << xpansion::logger::commons::create_str_mw(investment) << MW << std::endl;
    }
}
...
{
    _stream << std::endl;
    _stream << "--- Start sensitivity analysis with epsilon = " << epsilon << EUROS << " --- "
            << std::endl;
}

void SensitivityLogger::log_begin_pb_resolution(const SinglePbData& pb_data)
{
    _stream << indent_1 << "Solving " << pb_data.get_pb_description() << "..." << std::endl;
}
...
std::string format_objective(const SinglePbData& pb_data)
{
    ...
    {
        objective = xpansion::logger::commons::create_str_million_euros(pb_data.objective);
    }
    return objective;
}
...
void SensitivityLogger::log_pb_solution(const SinglePbData& pb_data)
{
    _stream << indent_1 << indent_1 << pb_data.get_pb_description() << " = "
            << format_objective(pb_data) << get_objective_unit(pb_data) << std::endl;
    _stream << indent_1 << indent_1 << "Overall cost = "
            << xpansion::logger::commons::create_str_million_euros(pb_data.system_cost)
            << MILLON_EUROS << std::endl;
    _stream << std::endl;
}
...
void SensitivityLogger::log_capex_summary(const std::vector<SinglePbData>& capex_data)
{
    double capex_min = 0;
    double capex_max = 0;
    ...
    _stream << indent_1 << "CAPEX interval: ["
            ...
            << xpansion::logger::commons::create_str_million_euros(capex_max) << "]" << MILLON_EUROS
            << std::endl
            << std::endl;
}
  ...
  const std::map<std::string, std::pair<double, double>>& candidates_bounds)
{
    ...
    for (const auto& kvp: investment_intervals)
    {
        ...
        _stream << indent_1 << indent_1 << kvp.first << ": ["
                ...
                << xpansion::logger::commons::create_str_mw(interval[MAX_C]) << "]" << MW
                << indent_1 << "-- possible interval = ["
                << xpansion::logger::commons::create_str_mw(candidates_bounds.at(kvp.first).first)
                << ", "
                << xpansion::logger::commons::create_str_mw(candidates_bounds.at(kvp.first).second)
                << "]" << MW << std::endl;
    }
    _stream << std::endl;
}
