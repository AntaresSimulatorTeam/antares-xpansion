...
SensitivityFileLogger::SensitivityFileLogger(const std::filesystem::path& filename)
{
    _file.open(filename);
    if (_file.fail())
    {
...
void SensitivityFileLogger::log_at_start(const SensitivityInputData& input_data)
{
    _userLog->log_at_start(input_data);
}

void SensitivityFileLogger::log_begin_pb_resolution(const SinglePbData& pb_data)
{
...
void SensitivityFileLogger::log_summary(const SensitivityInputData& input_data,
                                        const std::vector<SinglePbData>& pbs_data)
{
    _userLog->log_summary(input_data, pbs_data);
}

void SensitivityFileLogger::log_at_ending()
{
    _userLog->log_at_ending();
}
