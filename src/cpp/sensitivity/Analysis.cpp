...
Analysis::Analysis(SensitivityInputData input_data,
...
void Analysis::fill_single_pb_data(SinglePbData& pb_data, const RawPbData& raw_output) const
{
    pb_data.objective = raw_output.obj_value;
    pb_data.solver_status = raw_output.status;

    for (const auto& kvp: input_data.name_to_id)
    {
...
SinglePbData Analysis::run_optimization(SensitivityStudy::StudyType minimize)
{
    ...
    SinglePbData pb_data = {problem_type,
                            get_string_from_SensitivityPbType(problem_type),
                            candidate_name,
...
RawPbData Analysis::solve_sensitivity_pb() const
{
    ...
    else
    {
        ...
        sensitivity_pb_model->get_lp_sol(raw_output.solution.data(), nullptr, nullptr);
...
void Analysis::get_sensitivity_problem()
{
    ...
    {
        ProblemModifierCapex pb_modifier(input_data.epsilon,
                                         input_data.best_ub,
    ...
    {
        ProblemModifierProjection pb_modifier(input_data.epsilon,
                                              input_data.best_ub,
                                              input_data.last_master,
                                              input_data.name_to_id.at(candidate_name),
                                              candidate_name);
        sensitivity_pb_model = pb_modifier.changeProblem(nb_candidates);
        break;
    }
    ...
    {
        logger->display_message("Warning: Basis file " + input_data.basis_file_path.string()
                                + " could not be read.");
    }
}
