...
{
    LinkCapacitiesCSVWriter capacities_csv_writer(link_p, antares_link_folder_path);

    for (int i = 0; i < NUMBER_OF_HOUR_PER_YEAR; ++i)
    {
        if (auto chronicle_capacities_at_time = computeNewCapacitiesAllChronicles(investments_p,
                                                                                  link_p,
                                                                                  i);
            !chronicle_capacities_at_time.empty())
        {
            ...
            {
                capacities_csv_writer.WriteChronicleTimePoint(direct_capacities_at_time,
                                                              indirect_capacities_at_time);
            }
        }
...
int StudyUpdateLinkCapacitiesStrategy::Update(const ActiveLink& link,
  ...
  int timepoint_p) const
{
    ...
    for (int i = 1; i < link_p.number_of_chronicles() + 1; ++i)
    {
        ...
        for (const auto& candidate: candidates)
        {
            ...
            EnsureCandidateInvestmentFound(investments_p, link_p, candidate, it_candidate);
            double candidate_investment = it_candidate->second;
            direct_l += candidate_investment * candidate.directCapacityFactor(i, timepoint_p);
            indirect_l += candidate_investment * candidate.indirectCapacityFactor(i, timepoint_p);
        }
