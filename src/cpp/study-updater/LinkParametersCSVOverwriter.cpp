...
bool LinkParametersCSVOverWriter::open(const std::filesystem::path& linkdataFilename_l)
{
    ...
    if (!tempOutCsvFile.is_open())
    {
        (*logger_)(LogUtils::LOGLEVEL::ERR)
          << "ERROR: Error writing " + linkdataFilename_l.string() + "." << std::endl;
        return false;
    }
    ...
    if (!inputCsv_l_.is_open())
    {
        (*logger_)(LogUtils::LOGLEVEL::WARNING)
          << "WARNING: Missing file to update antares study : " << linkdataFilename_l << "."
          << " Unknown values were populated with 0 in a newly created file." << std::endl;
...
void LinkParametersCSVOverWriter::FillRecord(std::string basic_string_1, LinkdataRecord& record)
{
    ...
    else
    {
        ...
        {
            (*logger_)(LogUtils::LOGLEVEL::WARNING)
              << "WARNING: Missing entries in : " << link_parameters_file_path_.string()
