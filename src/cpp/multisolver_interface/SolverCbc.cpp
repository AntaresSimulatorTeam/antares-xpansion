...
void SolverCbc::write_prob_mps(const std::filesystem::path& filename)
{
    ...
    {
        const std::string& name(_clp_inner_solver.getRowName(i));
        if (name == "")
        {
    ...
    {
        const std::string& name(_clp_inner_solver.getColName(i));
        if (name == "")
        {
            std::stringstream buffer;
            buffer << "C" << i;
    ...
    writer.writeMps(filename_to_use.string().c_str(), 0 /*gzip it*/, 1, 1, nullptr, 0, nullptr);
}
...
{
    auto clp_updated_solver_ptr = dynamic_cast<OsiClpSolverInterface*>(_cbc.solver());
...
{
    int status = _clp_inner_solver.getModelPtr()->readBasis(filename.string().c_str());
...
void SolverCbc::get_rows(int* mstart,
                         int* mclind,
                         double* dmatval,
                         int size,
                         int* nels,
...
int SolverCbc::get_row_index(const std::string& name)
{
    int id = 0;
    int nrows = get_nrows();
    while (id < nrows)
...
int SolverCbc::get_col_index(const std::string& name)
{
...
void SolverCbc::add_cols(int newcol,
                         int newnz,
                         const double* objx,
                         const int* mstart,
                         const int* mrwind,
...
void SolverCbc::add_names(int type, const std::vector<std::string>& cnames, int first, int end)
{
    // TODO
    auto error = LOGLOCATION + "ERROR : addnames not implemented in the CLP interface.";
    throw NotImplementedFeatureSolverException(error);
...
void SolverCbc::chg_col_type(const std::vector<int>& mindex, const std::vector<char>& qctype)
{
...
void SolverCbc::chg_row_name(int id_row, const std::string& name)
{
    _clp_inner_solver.setRowName(id_row, name);
}

void SolverCbc::chg_col_name(int id_col, const std::string& name)
{
...
int SolverCbc::solve_mip()
{
    ...
    else if (_cbc.isProvenInfeasible() || _cbc.isInitialSolveProvenPrimalInfeasible())
    {
        lp_status = INFEASIBLE;
    }
    else if (_cbc.isProvenDualInfeasible() || _cbc.isInitialSolveProvenDualInfeasible())
    {
        lp_status = UNBOUNDED;
    }
    else
...
void SolverCbc::set_output_log_level(int loglevel)
{
    ...
    for (const auto message_handler: {_clp_inner_solver.messageHandler(), _cbc.messageHandler()})
    {
        if (loglevel > 1 && _fp)
        {
...
void SolverCbc::set_algorithm(const std::string& algo)
{
...
{
    throw InvalidSolverOptionException("set_simplex_iter : " + std::to_string(iter), LOGLOCATION);
}
