...
void fill_rows_from_COIN_matrix(const CoinPackedMatrix& matrix,
                                int* mstart,
                                int* mclind,
                                double* dmatval,
                                int* nels,
...
void fill_rhs_from_bounds(const double* rowLower,
                          const double* rowUpper,
                          double* rhs,
                          int first,
                          int last)
                                        ...
                                        const double* rhs)
{
    for (int i(0); i < newrows; i++)
    {
        ...
        else
        {
            ...
            buffer << LOGLOCATION << "ERROR : add rows, qrtype " << qrtype[i] << " of row " << i
                   << " to add unknown.";
