...
std::function<int(XPRSprob prob, const char* filename)> XPRSgetlasterror = nullptr;
...
std::function<int(XPRSprob prob, int ncols, const int colind[], const double objcoef[])>
  XPRSchgobj = nullptr;
std::function<int(XPRSprob prob, int objsense)> XPRSchgobjsense = nullptr;
std::function<
  int(XPRSprob prob, int nbounds, const int colind[], const char bndtype[], const double bndval[])>
  XPRSchgbounds = nullptr;
std::function<int(XPRSprob prob, int ncols, const int colind[], const char coltype[])>
  XPRSchgcoltype = nullptr;
std::function<int(XPRSprob prob, int nrows, const int rowind[], const double rhs[])>
  XPRSchgrhs = nullptr;
...
bool XpressLoader::LoadXpressFunctions(Solver::DynamicLibrary* xpress_dynamic_library)
{
    ...
    if (!notFound.empty())
    {
        std::string msg("Could not find the following functions (list may not be "
                        "exhaustive). ["
                        + StringJoin(notFound)
...
std::string XpressLoader::GetXpressVarFromEnvironmentVariables(const char* XPRESS_var, bool verbose)
{
    std::ostringstream msg;
    ...
    if (requiredSize != 0)
    {
        ...
        getenv_s(&requiredSize, xpress_home_from_env.data(), requiredSize, XPRESS_var);
    ...
    else
    {
        ...
        {
            msg.str("");
            msg << "[Windows getenv_s function]: " << XPRESS_var << " doesn't exist!\n";
            logger_->display_message(msg);
        }
    }
#else
...
std::vector<std::string> XpressLoader::XpressDynamicLibraryPotentialPaths()
{
    ...
    std::string xpress_home_from_env = GetXpressVarFromEnvironmentVariables(XPRESSDIR);
...
#else
        msg << "OS Not recognized by xpress/environment.cc." << " You won't be able to use Xpress.";
        logger_->display_message(msg);
#endif
    }
    else
    {
        msg.str("");
        msg << "Warning: " << "Environment variable " << XPRESSDIR << " undefined.\n";
        logger_->display_message(msg);
    }
...
#else
    msg.str("");
    msg << "OS Not recognized by environment.cc." << " You won't be able to use Xpress.";
    logger_->display_message(msg);
#endif
...
bool XpressLoader::LoadXpressDynamicLibrary(std::string& xpresspath)
{
    ...
    std::call_once(
      ...
      [this, &msg]()
      {
          const std::vector<std::string> canonical_paths = XpressDynamicLibraryPotentialPaths();
...
int XpressLoader::loadLicence(const std::string& lib_path, bool verbose)
{
    ...
    std::string xpauth_path_env_var = GetXpressVarFromEnvironmentVariables(XPAUTH_PATH, false);
    if (!xpauth_path_env_var.empty())
    {
        auto xpauth_path_parent_path = std::filesystem::path(xpauth_path_env_var)
                                         .parent_path()
                                         .string();
    ...
    std::string xpress_env_var = GetXpressVarFromEnvironmentVariables(XPRESS, false);
    ...
    // --- in xpress bin dir
    auto xpress_bin_dir = (std::filesystem::path(lib_path).parent_path() / "bin").string();
...
bool XpressLoader::initXpressEnv(bool verbose, int xpress_oem_license_key)
{
    ...
    if (xpress_oem_license_key == 0)
    {
        ...
        else
        {
            ...
            msg << "Xpress License error : " << errmsg << " (XPRSinit returned code " << code
                << "). Please check" << " environment variable XPRESS.\n";
            logger_->display_message(msg);
    ...
    else
    {
        ...
        {
            msg.str("");
            msg << "Warning: " << "Initialising xpress-MP with OEM key " << xpress_oem_license_key
                << "\n";
            logger_->display_message(msg);
        }
