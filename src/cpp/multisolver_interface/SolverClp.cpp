...
// #elif _WIN32
//     if ((_fp = _fsopen(_log_file.string().c_str(), "a+", _SH_DENYNO)) ==
//         nullptr)
// #endif
//     {
...
void SolverClp::read_prob_lp(const std::filesystem::path &filename) {
  ...
  *************************************************************************************************/
  ...
  void SolverClp::get_rows(int *mstart, int *mclind, double *dmatval, int size,
    ...
    coin_common::fill_rows_from_COIN_matrix(matrix, mstart, mclind, dmatval,
                                            nels, first, last);
  }
  ...
  void SolverClp::get_row_type(char *qrtype, int first, int last) const {
    ...
    coin_common::fill_row_type_from_row_bounds(rowLower, rowUpper, qrtype,
                                               first, last);
  }
  ...
  *************************************************************************************************/
  int SolverClp::solve_lp() {
    ...
    if (clp_status == CLP_OPTIMAL) {
      ...
      lp_status = UNKNOWN;
      std::cout << "Error : UNKNOWN CLP STATUS IS : " << clp_status
                << std::endl;
    }
    return lp_status;
  }
