...
SolverCplex::SolverCplex(FILE *fp) : SolverCplex() { _fp = fp; }
...
int SolverCplex::get_number_of_instances() { return _NumberOfProblems; }
...
void SolverCplex::init() {
  ...
  const std::string name =
      "DefaultProblem_" + std::to_string(_NumberOfProblems);
  _prb = CPXcreateprob(_env, &status, name.c_str());
  zero_status_check(status, "create problem");
}
...
void SolverCplex::get_rows(int *mstart, int *mclind, double *dmatval, int size,
                           int *nels, int first, int last) const {
  std::vector<int> surplus(1);
  int status = CPXgetrows(_env, _prb, nels, mstart, mclind, dmatval, size,
                          surplus.data(), first, last);
...
void SolverCplex::get_col_type(char *coltype, int first, int last) const {
  // Declaration en MIP pour creer les types de variables dans la memoire
  // (CPLEX)
...
void SolverCplex::add_rows(int newrows, int newnz, const char *qrtype,
...
void SolverCplex::add_cols(int newcol, int newnz, const double *objx,
...
void SolverCplex::chg_obj(const std::vector<int> &mindex,
...
void SolverCplex::chg_bounds(const std::vector<int> &mindex,
  ...
  int status = CPXchgbds(_env, _prb, mindex.size(), mindex.data(),
                         qbtype.data(), bnd.data());
  zero_status_check(status, "change bounds");
}

void SolverCplex::chg_col_type(const std::vector<int> &mindex,
                               const std::vector<char> &qctype) {
  assert(qctype.size() == mindex.size());
  int status =
      CPXchgctype(_env, _prb, mindex.size(), mindex.data(), qctype.data());
...
int SolverCplex::solve_lp() {
  ...
  if (cpx_status == CPX_STAT_OPTIMAL) {
    ...
    std::cout << "WARNING: Code CPX_STAT_OPTIMAL_INFEAS treated as optimal"
              << std::endl;
    lp_status = OPTIMAL;
...
int SolverCplex::get_splex_num_of_ite_last() const {
...
void SolverCplex::set_optimality_gap(double gap) {
  int status =
      CPXsetdblparam(_env, CPXPARAM_Simplex_Tolerances_Optimality, gap);
