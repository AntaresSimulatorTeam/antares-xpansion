...
int main(int argc, char **argv) {
  ...
  std::string path_to_log =
      options.OUTPUTROOT + PATH_SEPARATOR + "merge_mpsLog";
      ...
      for (auto const &x : kvp.second) {
        ...
        if (col_index == -1) {
          std::cerr << "missing variable " << x.first << " in " << kvp.first
                    ...
                    << ".";
          mergedSolver_l->write_prob_lp(options.OUTPUTROOT + PATH_SEPARATOR +
                                        "mergeError.lp");
          mergedSolver_l->write_prob_mps(options.OUTPUTROOT + PATH_SEPARATOR +
                                         "mergeError.mps");
          std::exit(1);
        } else {
