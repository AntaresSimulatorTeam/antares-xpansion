...
int main(int argc, char **argv) {
  ...
  try {
    ...
    std::string solver_to_use =
        (options.SOLVER_NAME == "COIN") ? "CBC" : options.SOLVER_NAME;
      ...
      for (auto const &x : kvp.second) {
        ...
        if (col_index == -1) {
          ...
          mergedSolver_l->write_prob_lp(
              (Path(options.OUTPUTROOT) / "mergeError.lp").get_str());
          mergedSolver_l->write_prob_mps(
              (Path(options.OUTPUTROOT) / "mergeError.mps").get_str());
          std::exit(1);
        } else {
    ...
    mergedSolver_l->write_prob_mps(
        (Path(options.OUTPUTROOT) / "log_merged.mps").get_str());
    LOG(INFO) << "Writting lp file" << std::endl;
    mergedSolver_l->write_prob_lp(
        (Path(options.OUTPUTROOT) / "log_merged.lp").get_str());
    ...
    mergedSolver_l->get_obj(obj_coef.data(), 0,
                            mergedSolver_l->get_ncols() - 1);
    ...
    jsonWriter_l.write(input.size(), overallCost_l, overallCost_l, investCost_l,
                       operationalCost_l, overallCost_l, x0, optimality_l);
    jsonWriter_l.dump(options.JSON_FILE);
