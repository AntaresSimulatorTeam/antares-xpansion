...
std::shared_ptr<SensitivityILogger> build_logger(const std::filesystem::path& log_file_path)
{
    auto master_logger = std::make_shared<SensitivityMasterLogger>();
    std::shared_ptr<SensitivityILogger> file_logger = std::make_shared<SensitivityFileLogger>(
      log_file_path);
    std::shared_ptr<SensitivityILogger> user_logger = std::make_shared<SensitivityLogger>(
      std::cout);
...
int main(int argc, char** argv)
{
    try
    {
        ...
        desc.add_options()("help,h", "produce help message")(
          "input,i",
          po::value<std::string>(&json_input_path)->required(),
          "path to the json input file")("output,o",
                                         po::value<std::filesystem::path>(&json_output_path),
                                         "path to the sensitivity json output file")(
          "benders,b",
          po::value<std::string>(&benders_output_path)->required(),
          "path to the benders json output file")(
          "master,m",
          po::value<std::string>(&last_master_path)->required(),
          "path to the last master mps file")(
          "basis",
          po::value<std::string>(&basis_path)->default_value(""),
          "path to the last master optimal basis file")(
          "structure,s",
          po::value<std::string>(&structure_path)->required(),
          "path to the structure txt file")("log,l",
                                            po::value<std::filesystem::path>(&log_path),
