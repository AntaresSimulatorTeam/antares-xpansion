...
int main(int argc, char **argv) {
  ...
  auto path_to_log =
      std::filesystem::path(options.OUTPUTROOT) / "bendersbybatchLog.txt.";
  google::SetLogDestination(google::GLOG_INFO, path_to_log.string().c_str());

  std::ostringstream oss_l = start_message(options, "Benders by Batch");
  LOG(INFO) << oss_l.str() << std::endl;

  const auto &loggerFileName =
      std::filesystem::path(options.OUTPUTROOT) / "reportbendersbybatch.txt";
  ...
  BendersByBatch benders(benders_options, logger, writer);
