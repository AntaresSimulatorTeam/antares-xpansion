...
int main(int argc, char **argv) {
  ...
  std::string loggerFileName =
      options.OUTPUTROOT + PATH_SEPARATOR + "reportbendersmpi.txt";
  if (world.rank() == 0) {
    Logger loggerFile =
  ...
  std::string path_to_log = options.OUTPUTROOT + PATH_SEPARATOR +
                            "bendersmpiLog-rank" +
  ...
  if (world.size() == 1) {
    ...
    if (world.rank() == 0) {
      ...
      LogData logData = defineLogDataFromBendersDataAndTrace(bendersMpi._data,
                                                             bendersMpi._trace);
      ...
      jsonWriter_l.write(input.size(), bendersMpi._trace, bendersMpi._data,
                         options.ABSOLUTE_GAP, options.RELATIVE_GAP,
