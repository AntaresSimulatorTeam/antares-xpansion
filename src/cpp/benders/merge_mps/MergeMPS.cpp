...
MergeMPS::MergeMPS(const MergeMPSOptions &options, Logger &logger, Writer writer) : _options(options), _logger(logger), _writer(writer) {}

void MergeMPS::launch()
{
    auto structure_path((Path(_options.INPUTROOT) / _options.STRUCTURE_FILE).get_str());
    CouplingMap input = build_input(structure_path, _options.SLAVE_NUMBER, _options.MASTER_NAME);
    ...
    for (auto const &kvp : input)
    {
        ...
        auto problem_name((Path(_options.INPUTROOT) / (kvp.first + MPS_SUFFIX)).get_str());
        ...
        for (auto const &x : kvp.second)
        {
            ...
            if (col_index == -1)
            {
                ...
                mergedSolver_l->write_prob_lp((Path(_options.OUTPUTROOT) / "mergeError.lp").get_str());
                mergedSolver_l->write_prob_mps((Path(_options.OUTPUTROOT) / ("mergeError" + MPS_SUFFIX)).get_str());
                std::exit(1);
            }
            else
            {
    ...
    mergedSolver_l->write_prob_mps((Path(_options.OUTPUTROOT) / ("log_merged" + MPS_SUFFIX)).get_str());
    LOG(INFO) << "Writting lp file" << std::endl;
    mergedSolver_l->write_prob_lp((Path(_options.OUTPUTROOT) / "log_merged.lp").get_str());
