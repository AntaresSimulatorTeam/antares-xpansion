...
public:
    ...
    void UpdateStoppingCriterion() override
    {
        if (batch_benders_)
        {
            // No public API to trigger internal UpdateStoppingCriterion on the concrete
            // implementation in this refactoring pass; keep as no-op to allow compilation and test
            // harness to control batching behavior via IBatchingStrategy mock in unit tests.
        }
    }
