	...
	if (!_input.empty())
	{
		...
		for (int islave(0); islave < _data.nslaves; ++islave, ++current_worker)
		{
			...
			else if (_world.rank() == current_worker)
			{
				...
				_map_slaves[kvp.first] = WorkerSlavePtr(new WorkerSlave(kvp.second, get_slave_path(kvp.first), slave_weight(_data.nslaves, kvp.first), _options.SOLVER_NAME, _options.LOG_LEVEL, _fp));
...
void BendersMpi::update_real_problem_list(std::vector<CouplingMap::const_iterator> &real_problem_list)
{
	...
	if (_world.rank() == 0)
	{
		...
		_master.reset(new WorkerMaster(it_master->second, get_master_path(), _options.SOLVER_NAME, _options.LOG_LEVEL, _data.nslaves, _fp));
