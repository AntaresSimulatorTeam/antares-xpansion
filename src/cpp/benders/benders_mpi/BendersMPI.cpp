...
BendersMpi::BendersMpi(BendersOptions const &options, Logger &logger,
    ...
    : BendersBase(options, logger), _exceptionRaised(false), _env(env),
      _world(world) {}
 ...
 *
 *  \param problem_list : map linking each problem name to its variables and
 * their id
 *
 */
...
void BendersMpi::load(CouplingMap const &problem_list) {
  ...
  if (!problem_list.empty()) {
    if (_world.rank() == 0) {
      ...
      _master.reset(new WorkerMaster(it_master->second,
                                     _options.get_master_path(), _options,
                                     _data.nslaves));
    ...
    for (int islave(0); islave < _data.nslaves; ++islave, ++current_worker) {
      ...
      if (_world.rank() == 0) {
        ...
        _map_slaves[kvp.first] = WorkerSlavePtr(new WorkerSlave(
            kvp.second, _options.get_slave_path(kvp.first),
            _options.slave_weight(_data.nslaves, kvp.first), _options));
...
/*!
 *  \brief Update the value of options RAND_AGGREGATION according to the number
 *of slaves on each thread
 *
 *	Update the value of options RAND_AGGREGATION according to the number of
 *slaves on each thread
 *
...
void BendersMpi::update_random_option() {
  ...
  int const n_add_rand(n_thread * !(n_slave_by_thread == n_rand_slave) +
                       n_max_slave * (n_slave_by_thread == n_rand_slave));
