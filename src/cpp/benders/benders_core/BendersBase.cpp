...
void BendersBase::print_csv() {
  std::string const output(_options.OUTPUTROOT + PATH_SEPARATOR +
...
void BendersBase::print_active_cut() {
  std::string output(_options.OUTPUTROOT + PATH_SEPARATOR +
                     "active_cut_output.csv");
  std::ofstream file(output, std::ios::out | std::ios::trunc);
  if (file) {
 ...
 *
 *  Method to solve and store optimal variables of random Slaves Problems after
 * fixing trial values
 *
 *  \param slave_cut_package : map storing for each slave its cut
 *
...
void BendersBase::compute_cut(AllCutPackage const &all_package) {
  for (int i(0); i < all_package.size(); i++) {
    for (auto const &itmap : all_package[i]) {
      ...
      if (_options.DELETE_CUT && !(_all_cuts_storage[itmap.first].find(cut) ==
                                   _all_cuts_storage[itmap.first].end())) {
        _data.deletedcut++;
      } else {
        ...
        if (_options.ACTIVECUTS) {
          _slave_cut_id[itmap.first].push_back(
              _master->get_number_constraint());
        }
...
void BendersBase::add_random_cuts(AllCutPackage const &all_package) {
  ...
  for (int i(0); i < all_package.size(); i++) {
    for (auto const &kvp : all_package[i]) {
      ...
      SlaveCutDataHandlerPtr const handler(
          new SlaveCutDataHandler(slave_cut_data));
      _master->add_cut_slave(_problem_to_id[kvp.first],
                             handler->get_subgradient(), _data.x0,
                             handler->get_dbl(SLAVE_COST));
      handler->get_dbl(ALPHA_I) = _data.alpha_i[_problem_to_id[kvp.first]];
      bound_simplex_iter(handler->get_int(SIMPLEXITER));
      if (handler->get_dbl(SLAVE_COST) <=
          _options.ABSOLUTE_GAP + handler->get_dbl(ALPHA_I)) {
        nboundslaves++;
      }
...
void BendersBase::dynamic_aggregation(AllCutPackage const &all_package) {
  if (_data.it == 1) {
    for (int i(0); i < _options.THRESHOLD_AGGREGATION; i++) {
      ...
      double sx0(0);
      double rhs(0);
      _dynamic_aggregate_cuts.push_back(
          std::tuple<Point, double, double>(s, sx0, rhs));
    }
  }
 ...
 *
 *	Delete every previous simple cut and replace them with the aggregated
 *ones
 *
 */
void BendersBase::gather_cut() {
  ...
  if (_options.THRESHOLD_AGGREGATION > 1) {
    for (int i(0); i < _dynamic_aggregate_cuts.size(); i++) {
      _master->add_dynamic_cut(std::get<0>(_dynamic_aggregate_cuts[i]),
                               std::get<1>(_dynamic_aggregate_cuts[i]),
                               std::get<2>(_dynamic_aggregate_cuts[i]));
    }
  } else if (_options.THRESHOLD_ITERATION > 1) {
    for (auto const &kvp : _problem_to_id) {
      _master->add_cut_by_iter(
          kvp.second, std::get<0>(_dynamic_aggregate_cuts[kvp.second]),
          std::get<1>(_dynamic_aggregate_cuts[kvp.second]),
          std::get<2>(_dynamic_aggregate_cuts[kvp.second]));
    }
  }
}
/*!
 ...
 *  \param all_package : storage of every slave information
 */
void BendersBase::store_current_aggregate_cut(
    AllCutPackage const &all_package) {
...
void BendersBase::compute_dynamic_cut_at_iter(
    const SlaveCutDataHandlerPtr &handler, int const nite) {
  for (auto const &kvp : _data.x0) {
    if (handler->get_subgradient().find(kvp.first) !=
        handler->get_subgradient().end()) {
      std::get<0>(_dynamic_aggregate_cuts[nite])[kvp.first] +=
          handler->get_subgradient()[kvp.first];
      std::get<1>(_dynamic_aggregate_cuts[nite]) +=
          handler->get_subgradient()[kvp.first] * kvp.second;
    }
  }
  ...
  for (int i(0); i < all_package.size(); i++) {
    for (auto const &itmap : all_package[i]) {
      ...
      for (auto const &kvp : handler->get_subgradient()) {
        std::get<0>(
            _dynamic_aggregate_cuts[_problem_to_id[itmap.first]])[kvp.first] +=
            kvp.second / _options.THRESHOLD_ITERATION;
        std::get<1>(_dynamic_aggregate_cuts[_problem_to_id[itmap.first]]) +=
            kvp.second * _data.x0.find(kvp.first)->second /
            _options.THRESHOLD_ITERATION;
      }
      std::get<2>(_dynamic_aggregate_cuts[_problem_to_id[itmap.first]]) +=
          handler->get_dbl(SLAVE_COST) / _options.THRESHOLD_ITERATION;
    }
  }
}
 ...
 *
 *  \param simplex_basis_package : map linking each slave to its current simplex
 * basis
 */
 ...
 *
 *  \param cut_id : map linking each cut from each slave to its id in master
 * problem
 *
...
void BendersBase::update_active_cuts() {
  ...
  for (auto &kvp : _slave_cut_id) {
    for (int i(0); i < kvp.second.size(); i++) {
      _active_cuts.push_back(std::make_tuple(
          _data.it, kvp.first, i + 1,
          (dual[kvp.first[i]] !=
           0))); //@TODO check : kvp.first is a string and indexing a vector
    }
  }
}
