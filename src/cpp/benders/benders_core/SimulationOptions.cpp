...
BendersBaseOptions SimulationOptions::get_benders_options() const
{
    ...
    {
        std::cerr << LOGLOCATION << "Invalid value " << MASTER_FORMULATION << " for option master"
                  << std::endl;
        std::exit(1);
    }
...
SimulationOptions::InvalidOptionFileException::InvalidOptionFileException(const std::string& arg):
    runtime_error(arg)
{
}
