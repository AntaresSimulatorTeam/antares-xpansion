...
std::vector<std::string> HeadersManager::HeadersList()
{
    ...
    if (type_ == HEADERSTYPE::LONG || method_ == BENDERSMETHOD::BENDERS_BY_BATCH)
    {
...
HeadersManagerExternalLoop::HeadersManagerExternalLoop(HEADERSTYPE type,
...
std::vector<std::string> HeadersManagerExternalLoop::HeadersList()
{
    ...
    std::move(base_headers.begin(), base_headers.end(), std::back_inserter(headers_list));
    return headers_list;
}
...
LogDestination::LogDestination(const std::filesystem::path& file_path, std::streamsize width):
    width_(width)
{
    ...
    else
    {
        ...
        err_msg << PrefixMessage(LogUtils::LOGLEVEL::WARNING, MATHLOGGERCONTEXT)
                << "Could not open the file: " << std::quoted(file_path.string().c_str()) << "\n";
...
LogDestination& MathLogger::LogsDestination()
{
    return log_destination_;
}

HEADERSTYPE MathLogger::HeadersType() const
{
    return type_;
}
...
{
    PrintBendersData(LogsDestination(), data, HeadersType(), BENDERSMETHOD::BENDERS);
}
...
double getDurationNotSolving(double iteration, double master, double subproblems)
{
...
{
    PrintExternalLoopData(LogsDestination(), data, HeadersType(), BENDERSMETHOD::BENDERS);
}
...
{
    auto type = HeadersType();
    HeadersManagerExternalLoop headers_manager(type, BENDERSMETHOD::BENDERS_BY_BATCH);
    MathLogger::setHeadersList(headers_manager.HeadersList());
}

void MathLoggerBendersByBatch::Print(const CurrentIterationData& data)
{
    PrintBendersData(LogsDestination(), data, HeadersType(), BENDERSMETHOD::BENDERS_BY_BATCH);
}

void MathLoggerBendersByBatchExternalLoop::Print(const CurrentIterationData& data)
{
    PrintExternalLoopData(LogsDestination(), data, HeadersType(), BENDERSMETHOD::BENDERS);
}

void MathLoggerDriver::add_logger(std::shared_ptr<MathLoggerImplementation> logger)
{
    if (logger)
    {
                                                   ...
                                                   HEADERSTYPE type)
{
    ...
    case BENDERSMETHOD::BENDERS:
        implementation_ = std::make_shared<MathLoggerBase>(file_path, width, type);
        break;
    case BENDERSMETHOD::BENDERS_OUTERLOOP:
        implementation_ = std::make_shared<MathLoggerBaseExternalLoop>(file_path, width, type);
        break;
    case BENDERSMETHOD::BENDERS_BY_BATCH:
        implementation_ = std::make_shared<MathLoggerBendersByBatch>(file_path, width, type);
        break;
    case BENDERSMETHOD::BENDERS_BY_BATCH_OUTERLOOP:
        implementation_ = std::make_shared<MathLoggerBendersByBatchExternalLoop>(file_path,
                                                                                 width,
                                                                                 type);
        break;
                                                   ...
                                                   HEADERSTYPE type)
{
    ...
    case BENDERSMETHOD::BENDERS:
        ...
        break;
    case BENDERSMETHOD::BENDERS_OUTERLOOP:
        implementation_ = std::make_shared<MathLoggerBaseExternalLoop>(width, type);
        break;
    case BENDERSMETHOD::BENDERS_BY_BATCH:
        implementation_ = std::make_shared<MathLoggerBendersByBatch>(width, type);
        break;
    case BENDERSMETHOD::BENDERS_BY_BATCH_OUTERLOOP:
        implementation_ = std::make_shared<MathLoggerBendersByBatchExternalLoop>(width, type);
        break;
...
MathLoggerImplementation::MathLoggerImplementation(std::shared_ptr<MathLogger> implementation):
