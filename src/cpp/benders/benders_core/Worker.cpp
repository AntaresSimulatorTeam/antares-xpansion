...
void Worker::solve(int &lp_status, const std::string &outputroot,
  ...
  if (lp_status != SOLVER_STATUS::OPTIMAL) {
    ...
    LOG(INFO) << "lp_status is : " << problem_status << std::endl;
    LOG(INFO) << "written in " << error_file_path.string() << std::endl;
    ...
    writer->dump();
    throw InvalidSolverStatusException("Invalid solver status " +
                                       problem_status + " optimality expected");
  }
