...
{
    explicit HeadersManagerExternalLoop(HEADERSTYPE type, const BENDERSMETHOD& method);
...
public:
    ...
    explicit LogDestination(const std::filesystem::path& file_path, std::streamsize width = 40);
...
struct MathLogger: public MathLoggerBehaviour
{
    ...
    explicit MathLogger(std::streamsize width = 40, HEADERSTYPE type = HEADERSTYPE::SHORT);
...
template<class T>
void MathLoggerExternalLoopSpecific<T>::display_message(const std::string& msg)
{
    // keep empty
}
