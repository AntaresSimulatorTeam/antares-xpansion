...
LastIterationReader::LastIterationReader(const std::filesystem::path& last_iteration_file):
...
LogData LastIterationReader::GetIterationData(const std::string& iteration_name)
{
    LogPoint x_cut;
    LogPoint min_invest;
    LogPoint max_invest;
    const auto candidates_array = last_iteration_file_content_[iteration_name]["candidates"];

    for (auto candidate = candidates_array.begin(); candidate != candidates_array.end();
         ++candidate)
    {
        x_cut.try_emplace((*candidate)["candidate"].asString(), (*candidate)["invest"].asDouble());
    ...
    return {last_iteration_file_content_[iteration_name]["lb"].asDouble(),
            ...
            last_iteration_file_content_[iteration_name]["subproblem_cost"].asDouble(),
            ...
            last_iteration_file_content_[iteration_name]["benders_elapsed_time"].asDouble(),
            last_iteration_file_content_[iteration_name]["master_duration"].asDouble(),
            last_iteration_file_content_[iteration_name]["subproblem_duration"].asDouble(),
            last_iteration_file_content_[iteration_name]["cumulative_number_of_subproblem_resolved"]
              .asInt()
