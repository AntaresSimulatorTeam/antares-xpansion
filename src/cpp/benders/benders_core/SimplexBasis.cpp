...
bool SimplexBasisHandler::operator<(SimplexBasisHandler const &other) const {
  return ((get_row() < other.get_row()) ||
          ((get_row() == other.get_row()) && (get_col() < other.get_col())));
}

/*!
 *  \brief Get basis status of the column in constraint matrix
 */
IntVector &SimplexBasisHandler::get_col() { return _data->second; }

/*!
 *  \brief Get basis status of each variable in the matrix
 */
IntVector &SimplexBasisHandler::get_row() { return _data->first; }

/*!
 *  \brief Get basis status of the column in constraint matrix
 */
IntVector const &SimplexBasisHandler::get_col() const { return _data->second; }

/*!
 *  \brief Get basis status of each variable in the matrix
 */
IntVector const &SimplexBasisHandler::get_row() const { return _data->first; }
