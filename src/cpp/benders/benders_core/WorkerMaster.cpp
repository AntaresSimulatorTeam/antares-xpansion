 ...
 */
int WorkerMaster::get_number_constraint() { return _solver->get_nrows(); }
...
void WorkerMaster::add_cut_by_iter(int const i, Point const &s,
                                   double const &sx0, double const &rhs) {
  // cut is -rhs >= alpha  + s^(x-x0)
  int ncoeffs(1 + (int)s.size());
  std::vector<char> rowtype(1, 'L');
  std::vector<double> rowrhs(1, 0);
...
WorkerMaster::WorkerMaster(Str2Int const &variable_map,
  ...
  // way to large as it appears in datas.max_invest. We set it to 1e20
