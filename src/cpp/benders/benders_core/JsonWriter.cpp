...
std::string timeToStr(std::time_t *time_p) {
  char buffer_l[100];
  strftime(buffer_l, sizeof(buffer_l), "%d-%m-%Y %H:%M:%S",
           std::localtime(time_p));
...
JsonWriter::JsonWriter() : _beginTime(std::time(0)), _endTime(std::time(0)) {}
...
std::string JsonWriter::getBegin() { return timeToStr(&_beginTime); }

std::string JsonWriter::getEnd() { return timeToStr(&_endTime); }

double JsonWriter::getDuration() { return std::difftime(_endTime, _beginTime); }

void JsonWriter::write(BendersOptions const &bendersOptions_p) {
// Options
#define BENDERS_OPTIONS_MACRO(name__, type__, default__)                       \
  _output["options"][#name__] = bendersOptions_p.name__;
...
void JsonWriter::write(int const &nbWeeks_p, BendersTrace const &bendersTrace_p,
                       ...
                       double const &min_abs_gap, double const &min_rel_gap,
                       double const &max_iter) {
  ...
  for (auto masterDataPtr_l : bendersTrace_p) {
    ...
    if (masterDataPtr_l->_valid) {
      ...
      _output["iterations"][strIterCnt_l]["optimality_gap"] =
          masterDataPtr_l->_bestub - masterDataPtr_l->_lb;
      _output["iterations"][strIterCnt_l]["relative_gap"] =
          (masterDataPtr_l->_bestub - masterDataPtr_l->_lb) /
          masterDataPtr_l->_bestub;
      _output["iterations"][strIterCnt_l]["investment_cost"] =
          masterDataPtr_l->_invest_cost;
      _output["iterations"][strIterCnt_l]["operational_cost"] =
          masterDataPtr_l->_operational_cost;
      _output["iterations"][strIterCnt_l]["overall_cost"] =
  ...
  if (bestItIndex_l >= 0 && bestItIndex_l < bendersTrace_p.size()) {
    _output["solution"]["investment_cost"] =
        bendersTrace_p[bestItIndex_l].get()->_invest_cost;
    _output["solution"]["operational_cost"] =
        bendersTrace_p[bestItIndex_l].get()->_operational_cost;
    _output["solution"]["overall_cost"] =
        bendersTrace_p[bestItIndex_l].get()->_invest_cost +
        bendersTrace_p[bestItIndex_l].get()->_operational_cost;

    for (auto pairNameValue_l : bendersTrace_p[bestItIndex_l]->get_point()) {
      _output["solution"]["values"][pairNameValue_l.first] =
          pairNameValue_l.second;
    }
  }
...
void JsonWriter::write(int nbWeeks_p, double const &lb_p, double const &ub_p,
                       double const &investCost_p,
                       double const &operationalCost_p,
                       double const &overallCost_p, Point const &solution_p,
...
void JsonWriter::dump(std::string const &filename_p) {
  ...
  if (jsonOut_l) {
    ...
    std::cout << "Impossible d'ouvrir le fichier json " << filename_p
              << std::endl;
  }
}
