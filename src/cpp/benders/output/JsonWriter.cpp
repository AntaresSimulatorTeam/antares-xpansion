...
namespace Output
{
JsonWriter::JsonWriter(std::shared_ptr<Clock> p_clock, const std::filesystem::path& json_filename):
    _clock(p_clock),
...
void JsonWriter::write_iteration(const Iteration& iter, const size_t iteration_num)
{
    ...
    _output[ITERATIONS_C][strIterCnt_l][SUBPROBLEM_DURATION_C] = iter.subproblem_duration;
    ...
    _output[ITERATIONS_C][strIterCnt_l][CUMULATIVE_NUMBER_OF_SUBPROBLEM_RESOLVED_C]
