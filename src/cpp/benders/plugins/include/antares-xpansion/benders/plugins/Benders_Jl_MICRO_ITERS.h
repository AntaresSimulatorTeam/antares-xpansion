...
{
    public : 
        Benders_Jl_MICRO_ITERS(const std::filesystem::path& input_root,const CouplingMap& coupling_map) ; 
        virtual ~Benders_Jl_MICRO_ITERS()  ;
        virtual void OnBendersStart(const SubproblemsMapPtr& subproblem_map,const Logger& logger,const BendersBaseOptions& options, const SolverLogManager& solver_log_manager)  ; 
        ...
        void BuildConstraintsReaderMap(const SubproblemsMapPtr& subproblem_map, const BendersBaseOptions& options,const SolverLogManager& solver_log_manager) ; 
