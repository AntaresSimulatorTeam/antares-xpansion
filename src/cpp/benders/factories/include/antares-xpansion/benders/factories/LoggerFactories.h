...
public:
    explicit FileAndStdoutLoggerFactory(const std::filesystem::path& report_file_path_string,
                                        bool expert_log_at_console)
    {
        auto masterLogger = std::make_shared<xpansion::logger::Master>();
        auto user_file = std::make_shared<xpansion::logger::UserFile>(report_file_path_string);
...
public:
                               ...
                               const std::filesystem::path& math_logs_file_path = "")
    {
        ...
        {
            auto math_logger_file = std::make_shared<MathLoggerFile>(method, math_logs_file_path);
