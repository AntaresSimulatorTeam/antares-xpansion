...
public:
    ...
    explicit BendersBaseAdapter(BendersBase* base = nullptr):
        base_(base)
    {
    }
    ...
    {
        return base_ ? base_->MasterRowIndex(row_name) : -1;
    }
    ...
    {
        return base_ ? base_->GetBestIterationData() : LogData{};
    }

    [[nodiscard]] WorkerMasterDataVect AllCuts() const override
    {
        return base_ ? base_->AllCuts() : WorkerMasterDataVect{};
    }
    ...
    {
        return base_ ? base_->BendersName() : std::string("BendersBaseAdapter");
    }

    [[nodiscard]] double execution_time() const override
    {
        return base_ ? base_->execution_time() : 0.0;
    }
