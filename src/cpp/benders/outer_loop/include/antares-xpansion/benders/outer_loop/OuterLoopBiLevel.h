...
public:
    ...
    bool Update_bilevel_data_if_feasible(const Point& x,
                                         const std::vector<double>& outer_loop_criterion,
                                         double overall_cost,
                                         double invest_cost_at_x,
                                         double lambda);

    void Init(const std::vector<double>& obj,
...
private:
    bool Check_bilevel_feasibility(const std::vector<double>& outer_loop_criterion,
                                   double overall_cost);
...
private:
    ...
    double lambda_ = 0.0;
    const std::vector<Benders::Criterion::CriterionSingleInputData>& outer_loop_input_data_;
};
} // namespace Outerloop
