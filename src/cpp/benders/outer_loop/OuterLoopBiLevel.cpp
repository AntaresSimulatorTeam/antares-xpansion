...
OuterLoopBiLevel::OuterLoopBiLevel(
  const std::vector<Benders::Criterion::CriterionSingleInputData>& outer_loop_input_data):
...
bool OuterLoopBiLevel::Update_bilevel_data_if_feasible(
  ...
  double lambda)
{
    if (found_feasible_ = Check_bilevel_feasibility(outer_loop_criterion, overall_cost);
        found_feasible_)
    {
...
bool OuterLoopBiLevel::Check_bilevel_feasibility(const std::vector<double>& outer_loop_criterions,
                                                 double overall_cost)
{
    return IsCriterionSatisfied(outer_loop_criterions) && overall_cost < bilevel_best_ub_;
...
bool OuterLoopBiLevel::IsCriterionSatisfied(const std::vector<double>& outer_loop_criterions)
{
    for (int index(0); index < outer_loop_criterions.size(); ++index)
    {
        if (outer_loop_criterions[index] > outer_loop_input_data_[index].Criterion())
        {
            return false;
        }
    }
...
void OuterLoopBiLevel::Update(const Point& x, double overall_cost, double invest_cost_at_x)
{
    bilevel_best_x_ = x;
...
void OuterLoopBiLevel::SetLambdaMaxToMaxInvestmentCosts(const std::vector<double>& obj,
                                                        const Point& max_invest,
                                                        const VariableMap& master_variable)
{
    lambda_max_ = 0;
...
const Point& OuterLoopBiLevel::BilevelBestX() const
{
    return bilevel_best_x_;
}
} // namespace Outerloop
