...
void CandidateLog::set_values_and_sizes(const LogData& data)
{
    for (const auto& [name, value]: data.x_cut)
    {
        ...
        valuesMap[INVEST_MIN] = get_formatted_string_from_value(data.min_invest.at(name));
        valuesMap[INVEST_MAX] = get_formatted_string_from_value(data.max_invest.at(name));
...
void CandidateLog::updateMaximumSizes(value_map& valuesMap)
...
{
    std::stringstream result;
    result << _line_prefix << indent_0 << indent_1 << std::setw(_sizes.at(CANDIDATE))
           << value.at(CANDIDATE);
