...
options_(options)
{
    ...
    std::filesystem::path micro_iterations_log_path = std::filesystem::path(options_.OUTPUTROOT)
                                                          / std::filesystem::path("micro_iterations_proc_" + std::to_string(world->rank())+".log"); 
    ...
    if (world->rank() == 0)
    {
        ...
        std::filesystem::path added_constraints_repo_path = std::filesystem::path(options_.OUTPUTROOT) / "added_constraints" ; 
