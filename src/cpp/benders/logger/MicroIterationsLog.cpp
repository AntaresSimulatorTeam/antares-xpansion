...
MicroIterationsLog::MicroIterationsLog(const SimulationOptions& options, std::map<std::string,std::string>& sub_constraints_map,std::map<std::string,std::vector<std::string>>&constraints_per_line,bool warm_start,mpi::communicator* world,int log_level)      
...
void MicroIterationsLog::Dump(int rank)
{
        ...
        mpi::gather(*_world,micro_iterations_per_benders_iter,micro_iterations_per_benders_iter_all_procs,0) ;
        ...
        if (rank == 0) 
        {
            for (int i=1; i<micro_iterations_per_benders_iter_all_procs.size(); i++)
            {
                ...
                {
                    for (auto& [sub_name, micro_iter_vec] :micro_iterations_per_benders_iter_all_procs[i][j] ) 
                    {
                        if (micro_iter_vec.size()>0) 
                        {
                            micro_iterations_per_benders_iter_all_procs[0][j][sub_name] = std::move(micro_iter_vec) ; 
                        }
                    }
                }
            }

        micro_iterations_per_benders_iter = std::move(micro_iterations_per_benders_iter_all_procs[0]) ; 
