...
void IterationResultLog::setValuesFromData(const LogData& data)
{
    ...
    _values.clear();
    _values.push_back(create_value_map("Operational cost",
                                       commons::create_str_million_euros(data.subproblem_cost),
                                       " Me"));
    _values.push_back(create_value_map("Investment cost",
                                       commons::create_str_million_euros(data.invest_cost),
                                       " Me"));
    _values.push_back(
      create_value_map("Overall cost", commons::create_str_million_euros(overall_cost), " Me"));
    _values.push_back(
      create_value_map("Best Solution", commons::create_str_million_euros(data.best_ub), " Me"));
    _values.push_back(
      create_value_map("Lower Bound", commons::create_str_million_euros(low_bd), " Me"));
...
void IterationResultLog::setMaximumStringSizes()
{ // Compute maximum string
  // size
                                                           ...
                                                           const size_map& sizes) const
{
    ...
    result << _line_prefix << indent_0 << indent_1 << std::setw(sizes.at(LABEL)) << value.at(LABEL);
    result << " = " << std::setw(sizes.at(VALUE)) << value.at(VALUE) << value.at(UNIT) << std::endl;
    return result.str();
}
