...
{
    _file << PrefixMessage(LogUtils::LOGLEVEL::INFO, CONTEXT) << "ITERATION " << it_number << ":"
          << std::endl;
    _file.flush();
}
...
{
    _file << PrefixMessage(LogUtils::LOGLEVEL::INFO, CONTEXT) << indent_1 << "Master solved in "
...
void UserFile::LogSubproblemsSolvingCumulativeCpuTime(double durationInSeconds)
{
    _file << PrefixMessage(LogUtils::LOGLEVEL::INFO, CONTEXT) << indent_1
...
void UserFile::log_at_ending(const LogData& d)
{
    ...
    _file << PrefixMessage(LogUtils::LOGLEVEL::INFO, CONTEXT) << indent_1 << "Best solution = it "
          << d.best_it << std::endl;
    _file.flush();
...
{
    _file << PrefixMessage(LogUtils::LOGLEVEL::INFO, CONTEXT) << "Restart Study..." << std::endl;
}
...
void UserFile::restart_best_iterations_infos(const LogData& best_iteration_data)
{
    _file << PrefixMessage(LogUtils::LOGLEVEL::INFO, CONTEXT) << indent_1
...
{
    _file << PrefixMessage(LogUtils::LOGLEVEL::INFO, CONTEXT) << indent_1
          << "cumulative number of call to solver (only for subproblems) : " << number << std::endl;
}
