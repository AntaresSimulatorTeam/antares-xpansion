...
{
    _stream << PrefixMessage(LogUtils::LOGLEVEL::INFO, CONTEXT) << str << std::endl;
}
...
{
    _stream << PrefixMessage(LogUtils::LOGLEVEL::INFO, CONTEXT) << "ITERATION " << it_number << ":"
            << std::endl;
}
...
{
    _stream
      << CandidateLog(PrefixMessage(LogUtils::LOGLEVEL::INFO, CONTEXT)).log_iteration_candidates(d);
}

void User::log_master_solving_duration(double durationInSeconds)
{
    _stream << PrefixMessage(LogUtils::LOGLEVEL::INFO, CONTEXT) << indent_1 << "Master solved in "
...
void User::LogSubproblemsSolvingWalltime(double durationInSeconds)
{
    _stream << PrefixMessage(LogUtils::LOGLEVEL::INFO, CONTEXT) << indent_1
            << "Subproblems solved in (walltime): " << durationInSeconds << " s" << std::endl;
}

void User::LogSubproblemsSolvingCumulativeCpuTime(double durationInSeconds)
{
    _stream << PrefixMessage(LogUtils::LOGLEVEL::INFO, CONTEXT) << indent_1
            << "Subproblems solved in (cumulative cpu): " << durationInSeconds << " s" << std::endl;
}

void User::log_at_iteration_end(const LogData& d)
{
    _stream << IterationResultLog(PrefixMessage(LogUtils::LOGLEVEL::INFO, CONTEXT))
...
void User::log_at_ending(const LogData& d)
{
    ...
    _stream << PrefixMessage(LogUtils::LOGLEVEL::INFO, CONTEXT) << indent_1 << "Best solution = it "
            << d.best_it << std::endl;
    _stream << PrefixMessage(LogUtils::LOGLEVEL::INFO, CONTEXT) << indent_1
...
{
    _stream << PrefixMessage(LogUtils::LOGLEVEL::INFO, CONTEXT) << "Benders ran in "
            << durationInSeconds << " s" << std::endl;
}
...
void User::log_stop_criterion_reached(const StoppingCriterion stopping_criterion)
{
    _stream << PrefixMessage(LogUtils::LOGLEVEL::INFO, CONTEXT)
            << "--- Run completed: " << criterion_to_str(stopping_criterion) << " reached"
            << std::endl;
}

void User::display_restart_message()
{
    _stream << PrefixMessage(LogUtils::LOGLEVEL::INFO, CONTEXT) << "Restart Study..." << std::endl;
}
...
{
    _stream << PrefixMessage(LogUtils::LOGLEVEL::INFO, CONTEXT)
            << "--- Relaxed gap reached, switch master formulation to integer" << std::endl;
}

void User::cumulative_number_of_sub_problem_solved(int number)
{
    _stream << PrefixMessage(LogUtils::LOGLEVEL::INFO, CONTEXT) << indent_1
            << "cumulative number of subproblem resolutions: " << number << std::endl;
}
} // namespace logger
} // namespace xpansion
