...
Benders::Benders(BendersOptions const &options, Logger &logger)
    : BendersBase(options, logger) {}
    ...
    for (int i(0); i < _data.nslaves; ++it) {
      if (it != it_master) {
        ...
        _map_slaves[it->first] = WorkerSlavePtr(new WorkerSlave(
            it->second, _options.get_slave_path(it->first),
 ...
 *
 *  Method to build Slaves cuts, store them in the Benders trace and add them to
 * the Master problem
 *
 */
