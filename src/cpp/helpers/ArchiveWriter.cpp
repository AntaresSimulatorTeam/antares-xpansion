...
int32_t ArchiveWriter::Open()
{
    ...
    std::unique_lock lock(mutex_);
    const auto err = mz_zip_writer_open_file(pmz_zip_writer_instance_,
                                             ArchivePath().string().c_str(),
                                             0 /* disk-spanning disabled */,
                                             1 /* append */);
    if (err != MZ_OK)
    {
        CloseUnsafe();
...
int32_t ArchiveWriter::AddFileInArchive(const FileBuffer& FileBufferToAdd)
{
    ...
    if (bw != len)
    {
        ...
        Delete();
        std::stringstream errMsg;
        errMsg << "[KO] mz_zip_writer_entry_write, expected " << len << "data to be read got" << bw
               << std::endl;
...
int32_t ArchiveWriter::AddFileInArchive(const std::filesystem::path& FileToAdd)
{
    std::unique_lock lock(mutex_);
...
int32_t ArchiveWriter::AddPathInArchive(const std::filesystem::path& path_to_add,
