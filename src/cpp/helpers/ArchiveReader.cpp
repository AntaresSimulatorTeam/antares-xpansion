...
int32_t ArchiveReader::Open()
{
    if (!std::filesystem::exists(ArchivePath()))
    {
        ...
        errMsg << "Archive: " << ArchivePath().string() << " does not exist" << std::endl;
        throw ArchiveIOGeneralException(MZ_EXIST_ERROR, errMsg.str(), LOGLOCATION);
    }
    auto err = mz_zip_reader_open_file(pmz_zip_reader_instance_, ArchivePath().string().c_str());
    if (err != MZ_OK)
    {
        Close();
        Delete();
    ...
    if (err != MZ_OK)
    {
        ...
        Delete();
        std::ostringstream errMsg;
        errMsg << "get underlying zip handle: " << ArchivePath().string() << std::endl;
        throw ArchiveIOGeneralException(err, errMsg.str(), LOGLOCATION);
    }
    return err;
}
...
int32_t ArchiveReader::ExtractFile(const std::filesystem::path& fileToExtractPath)
{
    return ExtractFile(fileToExtractPath,
                       ArchivePath().parent_path() / fileToExtractPath.filename());
}
                                   ...
                                   const std::filesystem::path& destination)
{
    ...
    err = mz_zip_reader_entry_save_file(pmz_zip_reader_instance_, targetFile.string().c_str());
    mz_zip_reader_entry_close(pmz_zip_reader_instance_);
    return err;
}

void ArchiveReader::LocateEntry(const std::filesystem::path& fileToExtractPath)
{
    auto err = mz_zip_reader_locate_entry(pmz_zip_reader_instance_,
                                          fileToExtractPath.string().c_str(),
                                          1);
    if (err != MZ_OK)
    {
        Close();
        ...
        errMsg << "File : " << fileToExtractPath.string().c_str()
               << " is not found in archive :" << ArchivePath().string().c_str() << std::endl;
...
void ArchiveReader::OpenEntry(const std::filesystem::path& fileToExtractPath)
{
    ...
    if (err != MZ_OK)
    {
        ...
        Delete();
        std::ostringstream errMsg;
        errMsg << "open " << fileToExtractPath.string() << " in archive :" << ArchivePath().string()
               << std::endl;
        throw ArchiveIOGeneralException(err, errMsg.str(), LOGLOCATION);
    }
}
  ...
  const std::filesystem::path& FileToExtractPath)
{
    ...
    LocateEntry(FileToExtractPath);
    OpenEntry(FileToExtractPath);
    int32_t len = mz_zip_reader_entry_save_buffer_length(pmz_zip_reader_instance_);
    std::vector<char> buf(len);
    auto err = mz_zip_reader_entry_save_buffer(pmz_zip_reader_instance_, buf.data(), len);
    if (err != MZ_OK)
    {
        Close();
        Delete();
...
uint64_t ArchiveReader::GetNumberOfEntries()
{
    ...
    if (auto err = mz_zip_get_number_entry(pzip_handle_, &number_entry); err != MZ_OK)
    {
        Close();
        Delete();
...
void ArchiveReader::LoadEntriesPath()
{
    if (!entries_path_are_loaded_)
    {
        ...
        if (err != MZ_OK)
        {
            ...
            Delete();
            std::ostringstream msg;
            msg << "to get first entry of archive" << ArchivePath().string().c_str() << std::endl;
            throw ArchiveIOGeneralException(err, msg.str(), LOGLOCATION);
        }
...
std::filesystem::path ArchiveReader::CurrentEntryPath()
{
    ...
    if (auto err = mz_zip_entry_get_info(pzip_handle_, &file_info); err != MZ_OK)
    {
        Close();
        Delete();
        std::ostringstream msg;
  ...
  const std::string& ext)
{
    ...
    std::copy_if(entries_path_.begin(),
                 entries_path_.end(),
                 std::back_inserter(result),
                 [&ext](auto& entry_path) -> bool
                 { return entry_path.extension().string() == ext; });
    return result;
}

std::vector<std::filesystem::path> ArchiveReader::ExtractPattern(const std::string& pattern,
    ...
    if (mz_zip_reader_goto_first_entry(pmz_zip_reader_instance_) == MZ_OK)
    {
        do
        {
            ...
            if (not_excluded)
            {
                ...
                mz_zip_reader_entry_save_file(pmz_zip_reader_instance_, target.string().c_str());
            }
