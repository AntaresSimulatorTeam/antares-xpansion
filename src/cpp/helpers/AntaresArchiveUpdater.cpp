...
bool isCriterionFile(const std::filesystem::path& file_name)
{
    ...
    return StringManip::StringUtils::contains(file_name_str, CRITERION_FILES_PREFIX)
...
{
    auto file_name_str = file_name.string();
    return StringManip::StringUtils::contains(file_name_str, VARIABLES_FILES_PREFIX)
           && !file_name.has_root_path();
}
...
{
    auto file_name_str = file_name.string();
    return StringManip::StringUtils::contains(file_name_str, CONSTRAINTS_FILES_PREFIX)
           && !file_name.has_root_path();
}
...
{
    auto file_name_str = file_name.string();
    return StringManip::StringUtils::contains(file_name_str, INTERCO_FILES_PREFIX)
           && !file_name.has_root_path();
}

bool IsAntaresMpsFile(const std::filesystem::path& file_name)
{
    return file_name.filename().extension() == MPS_FILES_EXTENSION && !file_name.has_parent_path();
}
...
int32_t AntaresArchiveUpdater::MinizipErase(void* reader, void* writer)
{
    ...
    while (err == MZ_OK)
    {
        ...
        if (!isCriterionFile(file_name) && !isConstraintsFile(file_name)
            && !isVariablesFile(file_name) && !isAreaFile(file_name) && !isIntercoFile(file_name)
            && !IsAntaresMpsFile(file_name))
        {
...
void AntaresArchiveUpdater::CleanAntaresArchive(const std::filesystem::path& src_archive,
                                                const std::filesystem::path& target_archive)
{
    auto tmp_target_path = target_archive;
    auto reuse_src_path = (target_archive == "" || target_archive.compare(src_archive) == 0);
    if (reuse_src_path)
    {
        tmp_target_path = src_archive.parent_path() / (src_archive.stem().string() + ".tmp.zip");
    }
    ...
    if (reuse_src_path)
    {
        std::filesystem::copy(tmp_target_path,
                              src_archive,
